...
//
// Revision Date  : $Date: 2014/08/11 18:03:54 $
//
...
//-----------------------------------------------------------------------------
///
/// Since mac like to move the exe around from to run, an address
/// difference must be used as this will continue to be the same
...
struct OpData 
{
  ...
  OpData(
    ...
    const N_LAS_Vector *          real_store_vector,
    const N_LAS_Vector *          imaginary_store_vector,
  ...
  int                           currentIndex_;
  const N_LAS_Vector *          realSolutionVector_;
  const N_LAS_Vector *          imaginarySolutionVector_;
  const N_LAS_Vector *          stateVector_;
  const N_LAS_Vector *          realStoreVector_;
  const N_LAS_Vector *          imaginaryStoreVector_;
...
/// Parallel::Machine argument ot the operator() function serves and the
/// communicator.
///
class Operator
{
...
Operator *makeOp(const BuilderManager &op_builder_manager, ParameterList::const_iterator &param_it);
...
inline void makeOps(Parallel::Machine comm, const BuilderManager &op_builder_manager, ParameterList::const_iterator begin, ParameterList::const_iterator end, std::back_insert_iterator<OpList> inserter)
{
  makeOps(comm, op_builder_manager, NetlistLocation(), begin, end, inserter);
}
