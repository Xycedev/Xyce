...
//-------------------------------------------------------------------------
// Filename       : $RCSfile: N_UTL_APFT.C,v $
//
...
//     }
  ...
  void N_UTL_APFT<std::vector<double> >::calculateIFT()
  {
    ...
    int signalLength_ = iftOutData_->size();
    Teuchos::SerialDenseVector<int,double> iftInDataVector ( Teuchos::View, &(*iftInData_)[1], signalLength_);
    Teuchos::SerialDenseVector<int,double> iftOutDataVector( Teuchos::View, &(*iftOutData_)[0], signalLength_);
