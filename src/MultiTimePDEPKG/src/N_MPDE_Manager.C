...
// ---------------------
// Revision Number: $Revision: 1.215.2.1 $
...
bool N_MPDE_Manager::run()
{
  ...
  else
  {
    ...
    //mpdeOnFlag_ = false;
    setMPDEOnFlag( false );
    bool ret1 = runInitialCondition_();
...
bool N_MPDE_Manager::setMPDEOptions(const N_UTL_OptionBlock & OB)
{
  ...
  for( ; iterPL != endPL; ++iterPL )
  {
    ...
    else if (tag == "OSCKICKOFF" ) //myhsieh, 081213
    {
      warpMPDEICFlag_ = iterPL->getImmutableValue<bool>();;
    }
    else
    {
...
//-----------------------------------------------------------------------------
// Function      : N_MPDE_Manager::initializeMPDEAll_
// Purpose       :
// Special Notes :
// Scope         : public
...
//-----------------------------------------------------------------------------
bool N_MPDE_Manager::initializeMPDEAll_(int size)
{
  bool returnValue = true;

  size_ = size; // Updated size_ with size of the good initial condition
                // determined in runInitialCondition_()
...
bool N_MPDE_Manager::runInitialCondition_()
{
  ...
  N_TIA_DataStore * dsPtr = anaIntPtr_->getTIADataStore();
  ...
  else
  {
   ...
   { 
    ...
    N_LAS_Vector &firstSolVec = *dsPtr->fastTimeSolutionVec[0];
    N_LAS_Vector &nextSolVec = *dsPtr->fastTimeSolutionVec[1];
    ...
    for (int i=0 ; i<n2-1 ; ++i) 
    {
        ...
        //if (!foundStart && firstSolVec[warpMPDEOSCOUT_] <=0 && nextSolVec[warpMPDEOSCOUT_] > 0) //110713
        if (!foundStart && firstSolVec[warpMPDEOSCOUT_] <=warpPhaseCoeff_ && nextSolVec[warpMPDEOSCOUT_] > warpPhaseCoeff_) 
        {
             goodStartPoint = i;
             foundStart = true;	    
        }
        //else if (foundStart && firstSolVec[warpMPDEOSCOUT_] <=0 && nextSolVec[warpMPDEOSCOUT_] > 0) //110713
    ...
    if (foundEnd == false)
    {
        std::cout <<"Not enough fast time points to find a good WaMPDE initial condition." << std::endl;
        std::cout <<"Try to icrease the fast time period, T2, to fix this error." << std::endl;
        exit(-1);
    }       
    ...
    initializeMPDEAll_(goodEndPoint - goodStartPoint); // initialize MPDE IC vectors to have
  ...
  case WAMPDE_IC_TRANSIENT:
    {
      ...
      if (warpMPDE_)
      {
        ...
        //size_ here has been updated b/c initializeMPDEAll_()
        // resize after the new fastTimes has been obtained 
        fastTimes_.resize(goodEndPoint - goodStartPoint);
      ...
      if (warpMPDE_) 
      {
        ...
        std::cout << "Beginning WAMPDE_IC_TRANSIENT: initialTime = " << tiaParams_.initialTime
          << " finalTime = " << tiaParams_.finalTime << std::endl;
        
        // register new tiaParams with time integrator... 
        tiaMPDEIfacePtr_->registerTIAParams(tiaParams_); 
      }
    }
    break;
...
bool N_MPDE_Manager::runTransientIC_()
{
  ...
  {
    Xyce::IO::ActiveOutput xyzzy(*outMgrPtr_);

    xyzzy.setStepSweep(std::vector<N_ANP_SweepParam>() );
    xyzzy.setDCSweep(std::vector<N_ANP_SweepParam>() );
    xyzzy.add(pdsMgrPtr_->getPDSComm()->comm(), Xyce::Analysis::ANP_MODE_TRANSIENT);

  {
    Xyce::IO::ActiveOutput x(*outMgrPtr_);

    if (saveIcData_)
    {
      x.add(Xyce::IO::PrintType::MPDE_IC, Xyce::Analysis::ANP_MODE_TRANSIENT);
    }
...
bool N_MPDE_Manager::setupMPDEProblem_()
{
  ...
  mpdeDeviceInterfacePtr_->registerAnalysisManager( anaIntPtr_ );
  ...
  nlsMgrPtr_.registerAnalysisManager( anaIntPtr_ );
  ...
  lasMPDESysPtr_.registerAnalysisManager( anaIntPtr_ );
  ...
  anaIntPtr_->initializeAll ( dynamic_cast<N_LOA_Loader*>( &*mpdeLoaderPtr_ ) );
...
bool N_MPDE_Manager::runTests_()
{
  ...
  RCP<N_LAS_Vector> B = rcp(mpdeBuilderPtr_->createVector());
  ...
  RCP<N_LAS_BlockVector> bB = rcp_dynamic_cast<N_LAS_BlockVector>(B);
  ...
  mpdeLoaderPtr_->loadDAEVectors(
      ...
      &*nextStore, &*currStore, &*lastStore, &*storeLeadCurrQComp, 
      &*Q, &*F, &*B, 
      0, 0 );
