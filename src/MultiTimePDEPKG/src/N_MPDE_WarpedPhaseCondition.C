...
//
// Revision Date  : $Date: 2014/04/22 22:50:13 $
//
// Current Owner  : $Author: myhsieh $
//-----------------------------------------------------------------------------
...
Teuchos::RefCountPtr<std::vector<int> > N_MPDE_WarpedPhaseCondition::getPhaseGraph()
{
  ...
  else if ( warpPhase_ == 1 )
  {
    ...
    //Genie 101013. Why isn't here omegaGID push_back?
  }
    ...
    )
{
  ...
  if (warpMPDEOSCOUTLID >= 0)
  {
    ...
    // genie 103113, Fix phase 3 for WaMPDE:
    // Look at norm instead of 2*h2
    // \omega*((\hat{x}_w(t_1,h2)-hat{x}_w(t_1,-h2))/(norm(\hat{x}_w(t_1,h2))) - alpha) = 0
      tmpPhaseValue = omega*(xwp - xwm)*(1.0 / max_xw) - warpPhaseCoeff_ ;
    }
     // genie 103113
  // Add phase condition 4 for WaMPDE based on the idea of slope change
  //
/*
  else if (warpPhase_ == 4)
  {
    int shiftp = offset_;
    int shiftm = (BlockCount-1)*offset_;
    xwpLID = bX.pmap()->globalToLocalIndex(warpMPDEOSCOUT_+shiftp); //hat{x}_w(t_1, h2)
    xw0LID = bX.pmap()->globalToLocalIndex(warpMPDEOSCOUT_); //hat{x}_w(t_1,0)
    xwmLID = bX.pmap()->globalToLocalIndex(warpMPDEOSCOUT_+shiftm); //hat{x}_w(t_1,-h2)
    double xwp = bX[xwpLID];
    double xw0 = bX[xw0LID];
    double xwm = bX[xwmLID];
    double prev_slope = wx0 - xwm;
    double next_slope = xwp - xw0;

    if ( pres_slope * next_slope < 0)
      //phaseValue = xw0 - warpPhaseCoeff_;
    // In this case, there is no return of phaseValue.
    // So this is not the place to put this code. Think about where to put it. 
  }
*/
    else
    {
