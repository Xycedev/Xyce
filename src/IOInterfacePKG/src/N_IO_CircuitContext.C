...
//
// Revision Date   : $Date: 2014/08/07 00:44:49 $
//
...
int CircuitContext::getTotalDeviceCount()
{
  ...
  {
    std::map<std::string,int> tmpDCMap( currentContextPtr_->localDeviceCountMap_ );
    ...
    for (instanceIter = start; instanceIter != end; ++instanceIter)
    {
      ...
      if (result)
      {
        ...
        std::map<std::string,int>::iterator lDCMb = currentContextPtr_->localDeviceCountMap_.begin();
        std::map<std::string,int>::iterator lDCMe = currentContextPtr_->localDeviceCountMap_.end();
        std::map<std::string,int>::iterator lDCMit = lDCMb;
      ...
      if (result)
      {
        std::list<std::string>::iterator findIter = 
          std::find((currentContextPtr_->usedContextList_).begin(),
                  (currentContextPtr_->usedContextList_).end(), *instanceIter);
        if (findIter == (currentContextPtr_->usedContextList_).end()) 
        {
    ...
    if (currentContextPtr_->parentContextPtr_ != NULL)
    {
      std::list<std::string>::iterator ucIter;
      std::list<std::string>::iterator uc_start = (currentContextPtr_->usedContextList_).begin();
      std::list<std::string>::iterator uc_end = (currentContextPtr_->usedContextList_).end();

      for (ucIter = uc_start; ucIter!=uc_end; ++ucIter)
      {
        std::list<std::string>::iterator findIter = 
          std::find((currentContextPtr_->parentContextPtr_->usedContextList_).begin(),
                    (currentContextPtr_->parentContextPtr_->usedContextList_).end(), *ucIter);
        if (findIter == (currentContextPtr_->parentContextPtr_->usedContextList_).end()) 
...
//----------------------------------------------------------------------------
void CircuitContext::pruneContexts( std::list<std::string>& keepContexts )
{
  std::map< std::string, CircuitContext * >::iterator itsc2, itsc = circuitContextTable_.begin();
  ...
  while ( itsc != circuitContextTable_.end() )
  {
    ...
    std::list<std::string>::iterator findIter = std::find(keepContexts.begin(), keepContexts.end(), itsc->first);
    if (findIter == keepContexts.end())
    {
