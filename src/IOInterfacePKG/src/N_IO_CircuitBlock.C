...
// ---------------------
//
// Revision Number: $Revision: 1.307 $
//
...
CircuitBlock::~CircuitBlock()
{
  if (useCount_ == 1)
  {
    ...
    {
      //Destroy the SSFs 
      std::map<std::string,FileSSFPair>::iterator iterSSF = ssfMap_.begin();
      std::map<std::string,FileSSFPair>::iterator endSSF = ssfMap_.end();
    ...
    Xyce::Topo::InsertionTool* insertionToolPtr)
{
  ...
  {
    insertionToolPtr_ = insertionToolPtr;
  }
}
  ...
  Device::DeviceInterface* devIntPtr)
{
  ...
  {
    devIntPtr_ = devIntPtr;
  }
}

//----------------------------------------------------------------------------
// Function       : CircuitBlock::getFileLocation
// Purpose        :
// Special Notes  :
// Scope          :
// Creator        : Lon Waters
...
//----------------------------------------------------------------------------
bool CircuitBlock::getFileLocation( std::string const& path,
...
bool CircuitBlock::parseNetlistFilePass1( const std::string &libSelect, std::string libInside )
{
  ...
  if ( parentCircuitPtr_ == NULL )
  {
    ...
    {
      Report::UserError0() << "Could not find netlist file " << netlistFileName_;
      return false;
    }

    ssfPtr_ = new
      SpiceSeparatedFieldTool(netlistIn_, netlistFileName_, externalNetlistParams_);
    ssfMap_[netlistFileName_] = FileSSFPair((std::ifstream*)0, ssfPtr_);

    char ch_input[256];
    readline( netlistIn_, ch_input );
    title_ = ch_input;
    ...
    parsePreprocess(netlistFileName_);
  ...
  for (;;) {
    ...
    if (handleLinePass1( line_parsed, par, fun, modMap_, sub, libSelect, libInside ) )
  ...
  if( ( parentCircuitPtr_ == NULL ) && ( !(rawMIs_.empty()) ) )
  {
    ...
    for( ; mm != rawMIs_.end(); ++mm )
    {
      ...
      circuitContext_.resolve(junkSubcircuitParams);

      // Parses the K line
      //( ( *mm ).second ).extractData( &circuitContext_, &metadata_ );
  ...
  // Then register final objects with the distribution tool for broadcasting.
  if ( parentCircuitPtr_ == NULL )
  {
    ...
    // Get the total device count for the circuit, inform the distribution tool.
    int count = circuitContext_.getTotalDeviceCount();
    distToolPtr_->deviceCount( count );
    
    // Collect the device types from the circuit context if we are only performing syntax checking.
    if ( !netlistSave_ )
    {
      devIntPtr_->addDevicesToCount( circuitContext_.getDeviceCountMap() );
    }
 
    // Get the name of all the subcircuit instances that were used in computing the device count. 
    std::list<std::string> usedSubcircuits = circuitContext_.getUsedContextList();
    ...
    // Register the circuit context and options table with the distribution tool.

    // Set the circuit context.
    distToolPtr_->setCircuitContext(&circuitContext_);
  
    // Set the circuit options.
    distToolPtr_->setCircuitOptions(optionsTable_);
...
bool CircuitBlock::parseNetlistFilePass2()
{
  ...
  if (DEBUG_IO) {
    if (parentCircuitPtr_ == NULL)
    {
      Xyce::dout() << "Pass 2 parsing of netlist file: " <<
        netlistFileName_ << std::endl;
    }
  }

  distToolPtr_->setFileName(netlistFileName_);
  ...
  if( circuitContext_.haveMutualInductances() )
  {
    ...
    for( int i = 0; i < n; ++i )
    {
      ...
      if( !distToolPtr_->circuitDeviceLine(
        circuitContext_.getMILine( i ) ) )
      {
        handleDeviceLine(
         circuitContext_.getMILine( i ), libSelect, libInside );
      }
    }
  }
  ...
  if (DEBUG_IO) {
    ...
    std::map<std::string,std::string,LessNoCase>::iterator currentAliasIt_ = aliasNodeMap_.begin();
    std::map<std::string,std::string,LessNoCase>::iterator endAliasIt_ = aliasNodeMap_.end();
...
void CircuitBlock::writeOutNetlist()
{
  ...
  if (commandLine_.getHangingResistor().getNetlistCopy())
  {
    ssfPtr_->setLocation(fileStartPosition_);
    ssfPtr_->setLineNumber( lineStartPosition_ );
    netlistIn_.clear();
    netlistIn_.seekg(0, std::ios::beg);
    ssfPtr_->changeCursorLineNumber( 1 );
    produceUnflattenedNetlist(netlistFileName_);
...
void CircuitBlock::print()
{
  ...
  {
    Xyce::dout() << "Circuit Name:  " << name_ << std::endl;
  }
  Xyce::dout() << std::endl;
...
bool CircuitBlock::extractSubcircuitData(std::string fileName, int lineNum)
{
  ...
  if ( numFields < 3 )
  {
    Report::DevelFatal0().in("CircuitBlock::extractSubcircuitData").at(netlistFileName_, parsedLine_[0].lineNumber_)
      << "This should have been detected earlier in parse";
    return false;
  }

  // Extract the subcircuit name.
  ExtendedString field ( parsedLine_[1].string_ );
  field.toUpper();
  name_ = field;
  ...
  parsedLine_.clear();
  std::vector<SpiceSeparatedFieldTool::StringToken>(parsedLine_).swap(parsedLine_);
...
bool CircuitBlock::instantiateDevices(std::string libSelect, std::string libInside)
{
  ...
  while (getLinePass2(line, libSelect, libInside))
  {
    // parse locally if distool does not distribute
    if( !distToolPtr_->circuitDeviceLine( line ) )
    {
...
// Creation Date : Wed May 14 14:19:20 2014
//-----------------------------------------------------------------------------
///
/// Add any globals to the device manager global parameter list.
///
void
CircuitBlock::registerGlobalParams() const
{
  const OptionBlock *globals = circuitContext_.getGlobals();
  for (std::list<Util::Param>::const_iterator it = globals->optionData.begin(), end = globals->optionData.end(); it != end; ++it)
  {
    devIntPtr_->addGlobalPar(*it);
  }
}
...
//----------------------------------------------------------------------------
bool CircuitBlock::instantiateDevice(
  DeviceBlock &                                 device,
  std::string &                                 prefix,
  ...
  const std::string &                           libInside)
{
    ...
    for (; nodeIter != nodeIterEnd; ++nodeIter)
    {
      ...
      if (nodeMapIter != nodeMap.end())
      {
        ...
        nodeValues.push_back(tagged_param(nodeMapIter->second,0));
               ...
               !circuitContext_.globalNode(nodeIter->tag))
      {
  ...
  if (!device.isSubcircuitInstance())
  {
    ...
    if (device.getModelName() != "")
    {
      ...
      // ERK: tmpModel_ is a persistant object in the CircuitBlock class.
      ...
      mainCircuitPtr_->addModel(tmpModel_, modelPrefix);
    }
  ...
  for (std::vector<std::string>::const_iterator nName = names.begin(); nName != names.end(); ++nName)
  {
   // find duplicates across procs
    ...
    if (iterDN != deviceNames_.end())
    {
      ...
      {
        Report::UserError().at(deviceNames_[*nName]->getNetlistLocation())
          << "Duplicate device " << *nName;
      }
    }
  }
}
...
void CircuitBlock::addTableData( DeviceBlock & device )
{
  ...
  std::string dName = iBPtr->getInstanceName().getEncodedName();
  ...
  if (netlistSave_)
  {
    ...
    if (DEBUG_IO) {
      ...
      Xyce::dout() << "Inserting device: " << dName
                   ...
                   << " from file: " << iBPtr->getNetlistLocation().getPath()
                   << " line: " << iBPtr->getNetlistLocation().getLineNumber() << std::endl;
    }

    insertionToolPtr_->insertNode(device.getDeviceData().getNodeBlock(), iBPtr);
  }
  ...
  {
    Report::UserError().at(iBPtr->getNetlistLocation()) << "Duplicate device "<< dName;
  }
...
void CircuitBlock::addModel( ParameterBlock & model, std::string const& modelPrefix)
{
  ...
  std::map<std::string,int>::iterator iterMN = modelNames_.find(modelName);

  if ( iterMN == modelNames_.end() )
  {
    modelNames_.insert(mnMap::value_type(modelName,0));
    devIntPtr_->addDeviceModel(model.modelData);
  }
}
...
void CircuitBlock::addOptions( OptionBlock const& options )
{
  ...
  {
    const Util::OptionBlock & optionData = options.optionData;
    std::list<Util::Param>::const_iterator paramIter;
    std::list<Util::Param>::const_iterator paramEnd;
    paramIter = optionData.getParams().begin();
    paramEnd = optionData.getParams().end();

    for ( ; paramIter != paramEnd; ++paramIter)
    {
      aliasNodeMapHelper_.insert( std::string( ( *paramIter).uTag() ) );
    }
  }

  else if ( options.getName() == "DEVICE" )
  {
    deviceOptions_ = options.optionData;
  }
  else if (options.getName() == "OUTPUT-LINE")
  {
    ...
    std::list<Util::OptionBlock>::iterator first = optionsTable_.begin();
    std::list<Util::OptionBlock>::iterator last = optionsTable_.end();
    ...
    else
    {
      ...
      optionBlockIter->getParams().push_back(*paramIter);
      paramIter++;
      optionBlockIter->getParams().push_back(*paramIter);
    }
  ...
  optionsTable_.push_back( options.optionData );
}
...
bool CircuitBlock::handleLinePass1(
  bool & result, std::map<std::string,int> & par, std::map<std::string,int> & fun,
  ...
  const std::string &libSelect, std::string &libInside )
{
  ...
  if (lineType >= 'A' && lineType <= 'Z')
  {
    ...
    if (!removecomponent)
    {
      if (lineType != 'X' && lineType != 'K')
      {
        ...
        std::string ES2 ( ES1.substr(0,1) ); 
        if (lineType == 'Y')
        {
          // Take remainder of string after "Y" as the device name.
          ES2 = ES1.substr(1);
        }
      ...
      if (lineType == 'X')
      {
        ...
        circuitContext_.addInstance(device.getModelName(),netlistFileName_,line[0].lineNumber_);
      }
      if (lineType == 'K')
      {
        circuitContext_.incrementDeviceCount( ES1.substr(0,1) );
        DeviceBlock device(circuitContext_, metadata_, netlistFileName_, line);
  ...
  else if (lineType == '.')
  {
    ...
    else if (ES1 == ".ENDS")
    {
      ...
      setEndPosition();
      circuitContext_.endSubcircuitContext();
      return false;
    }
    else if (ES1 == ".FUNC")
    {
    ...
    else if (ES1 == ".INCLUDE" || ES1 == ".INC" || ES1 == ".LIB")
    {
      ...
      if (includeFile != "")
      {
        result = parseIncludeFile(includeFile, libSelect_child, par, fun, modMap, sub);
      }
    }
    else if (ES1 == ".ENDL")
    {
    ...
    else if (ES1 == ".MODEL")
    {
      ...
      result = modelPtr->extractModelData( metadata_ );
    ...
    {
      if ( ES1 == ".GLOBAL_PARAM" && parentCircuitPtr_ != NULL )
      {
        Report::UserError().at(netlistFileName_, line[0].lineNumber_)
        ...
        OptionBlock param( netlistFileName_, line, metadata_ );
        result = param.extractData();
        if (result)
        {
          const Util::OptionBlock & optionData = param.optionData;
          std::list<Util::Param>::const_iterator paramIter;
          std::list<Util::Param>::const_iterator paramEnd;
          paramIter = optionData.getParams().begin();
          paramEnd = optionData.getParams().end();
        ...
        if (ES1 == ".PARAM")
          circuitContext_.addParams( param );
        else
          circuitContext_.addGlobalParams( param );
      }
    }
    else if (ES1 == ".GLOBAL")
    {
    ...
    else if (ES1 == ".SUBCKT")
    {
      ...
      if (result) {
        result = subcircuitBlockPtr->extractSubcircuitData(netlistFileName_, line[0].lineNumber_)
                 && result;
...
//----------------------------------------------------------------------------
// Function       : CircuitBlock::getLinePass2
// Purpose        :
// Special Notes  :
// Scope          : private
// Creator        : Lon Waters
...
//----------------------------------------------------------------------------
bool CircuitBlock::getLinePass2(
    std::vector<SpiceSeparatedFieldTool::StringToken> & line,
    const std::string &libSelect, std::string &libInside)
{
  int eof = 0;
      ...
      if (lineType == 'L')
      {
        ...
        for( int i = 0; i < numMIs; ++i )
        {
          if( MIs[i].inductors.count( ES1 ) )
          {
            ...
            device_.setFileName(netlistFileName_);
            device_.extractData( );
            ...
            for( int j = 0; j < numParams; ++j )
            {
              ...
              if( param.uTag() == "L" )
              {
                ...
                std::list<tagged_param>::const_iterator paramIter=device_.getNodeValues().begin();
                ( MIs[i].terminals[ES1] ).push_back( paramIter->tag );
                ++paramIter;
                ( MIs[i].terminals[ES1] ).push_back( paramIter->tag );
              }
            }
    ...
    const std::string &libSelect, const std::string &libInside)
{
  ...
  device_.clear();
  device_.setParsedLine(deviceLine);
  device_.setFileName(netlistFileName_);
  std::string prefix(circuitContext_.getPrefix());
  std::map<std::string, std::string> * nodeMapPtr = circuitContext_.getNodeMapPtr();
  result = instantiateDevice(device_, prefix, *nodeMapPtr, libSelect, libInside);

  return result;
}

//--------------------------------------------------------------------------
// Function      : CircuitBlock::expandSubcircuitInstance
...
//--------------------------------------------------------------------------
bool CircuitBlock::expandSubcircuitInstance(
  ...
  const std::string &   libInside)
{
  ...
  // Find the subcircuit corresponding to this instance.
  CircuitBlock* subcircuitPtr = findSubcircuit(subcircuitInstance.getModelName());
  if ( subcircuitPtr == NULL )
  {
    Report::UserError0().at(netlistFileName_, subcircuitInstance.getParsedLine()[0].lineNumber_)
  ...
  { // The subcircuit is in an include file.
    // Get SSF from Pass 1's ssf map
    if( ssfMap_.count( subcircuitPtr->netlistFileName_ ) )
      newssf = ssfMap_[subcircuitPtr->netlistFileName_].second;
    else
    {
      distToolPtr_->endDeviceLines();
      Report::UserError().at(netlistFileName_, subcircuitInstance.getParsedLine()[0].lineNumber_)
        << "Can't find include file " << subcircuitPtr->netlistFileName_;
    }
  }
...
bool CircuitBlock::resolveExpressionsInOptionBlocks()
{
  ...
  std::list<Util::OptionBlock>::iterator currentOptionBlockIter = optionsTable_.begin();
  std::list<Util::OptionBlock>::iterator endOptionBlockIter = optionsTable_.end();
    ...
    if (currentOptionBlockIter->getName() == "PRINT")
    {
      ...
      while( currentParamIt != endParamIt )
      {
        if( Util::hasExpressionTag(*currentParamIt) )
        {
          ...
          circuitContext_.resolveParameter(*currentParamIt,exceptionStrings);
...
//--------------------------------------------------------------------------
// Function      : CircuitBlock::handleIncludeLine
...
//--------------------------------------------------------------------------
void CircuitBlock::handleIncludeLine(
...
//--------------------------------------------------------------------------
// Function      : CircuitBlock::handleStd::EndlLine
// Purpose       : Handle a netlist .endl line
// Special Notes :
// Creator       : Dave Shirley, PSSI
// Creation Date : 10/09/2009
//--------------------------------------------------------------------------
void CircuitBlock::handleEndlLine(
      std::vector<SpiceSeparatedFieldTool::StringToken> const& parsedLine,
      const std::string& libSelect, std::string& libInside)
{
  if (libInside == "")
  ...
  if ( parsedLine.size() >= 2 )
  {
    ...
    if ( parsedLine.size() >2 )
    {
      Report::UserWarning().at(netlistFileName_, parsedLine[0].lineNumber_)
  ...
  ModelMap & modMap, std::map<std::string,int> & sub)
{
  ...
  std::string old_netlistFileName_(netlistFileName_);
  ...
  for (;;) {
    ...
    if (handleLinePass1(line_parsed, par, fun, modMap, sub, libSelect, libInside) )
      result = result && line_parsed;
    else
      break;
  }
  ...
  netlistFileName_ = old_netlistFileName_;
...
//--------------------------------------------------------------------------
// Function      : CircuitBlock::parseIncludeFile2
// Purpose       : Jump to include file for 2nd pass
// Special Notes :
// Creator       : Rob Hoekstra
// Creation Date : 08/02/2004
//--------------------------------------------------------------------------
bool CircuitBlock::parseIncludeFile2(std::string const& includeFile,
             const std::string &libSelect)
{
  // Save current ssfPtr_ and netlistFileName_.
  SpiceSeparatedFieldTool* oldssfPtr = ssfPtr_;
  ...
  int result = instantiateDevices(libSelect, std::string(""));
  if (!result)
    return result;
  ...
  ssfPtr_ = oldssfPtr;
  netlistFileName_ = old_netlistFileName_;
  distToolPtr_->setFileName(old_netlistFileName_);
...
// Creation Date :
//--------------------------------------------------------------------------
void CircuitBlock::handleInitCond(
 std::vector< SpiceSeparatedFieldTool::StringToken > const& parsedLine )
{
...
bool CircuitBlock::handleAnalysis()
{
  ...
  analysisName_ = op_analysis_it->getName();
...
//----------------------------------------------------------------------------
// Function       : CircuitBlock::handleMutualInductance
...
//----------------------------------------------------------------------------
bool CircuitBlock::handleMutualInductance( DeviceBlock & device )
{
...
//--------------------------------------------------------------------------
bool CircuitBlock::parsePreprocess(const std::string & netlistFileName_)
{
  if (DEBUG_IO)
...
//--------------------------------------------------------------------------
// Function      : CircuitBlock::removeTwoTerminalDevice
...
//--------------------------------------------------------------------------
bool CircuitBlock::removeTwoTerminalDevice(const char linetype, const
...
//--------------------------------------------------------------------------
// Function      : CircuitBlock::removeThreeTerminalDevice
...
//--------------------------------------------------------------------------
bool CircuitBlock::removeThreeTerminalDevice(const char linetype,
...
// Creation Date : 10/10/2007
//--------------------------------------------------------------------------
bool CircuitBlock::removeC()
{
  return remove_redundant_C_;
}

//--------------------------------------------------------------------------
// Function      : CircuitBlock::removeD
...
// Creation Date : 10/10/2007
//--------------------------------------------------------------------------
bool CircuitBlock::removeD()
{
  return remove_redundant_D_;
}

//--------------------------------------------------------------------------
// Function      : CircuitBlock::removeI
...
// Creation Date : 10/10/2007
//--------------------------------------------------------------------------
bool CircuitBlock::removeI()
{
  return remove_redundant_I_;
}

//--------------------------------------------------------------------------
// Function      : CircuitBlock::removeL
...
// Creation Date : 10/10/2007
//--------------------------------------------------------------------------
bool CircuitBlock::removeL()
{
  return remove_redundant_L_;
}

//--------------------------------------------------------------------------
// Function      : CircuitBlock::removeM
...
// Creation Date : 10/10/2007
//--------------------------------------------------------------------------
bool CircuitBlock::removeM()
{
  return remove_redundant_M_;
}

//--------------------------------------------------------------------------
// Function      : CircuitBlock::removeQ
...
// Creation Date : 10/10/2007
//--------------------------------------------------------------------------
bool CircuitBlock::removeQ()
{
  return remove_redundant_Q_;
}

//--------------------------------------------------------------------------
// Function      : CircuitBlock::removeR
...
// Creation Date : 10/10/2007
//--------------------------------------------------------------------------
bool CircuitBlock::removeR()
{
  return remove_redundant_R_;
}

//--------------------------------------------------------------------------
// Function      : CircuitBlock::removeV
...
// Creation Date : 10/10/2007
//--------------------------------------------------------------------------
bool CircuitBlock::removeV()
{
  return remove_redundant_V_;
}

//--------------------------------------------------------------------------
// Function      : CircuitBlock::replgnd
...
//--------------------------------------------------------------------------
void CircuitBlock::produceUnflattenedNetlist(const std::string & netlistFileName_)
{
...
void checkNodeDevConflicts(CircuitBlock* cktBlk, N_PDS_Comm* pdsComm)
{
  ...
  if ( numProcs != 1 )
  {
    ...
    byteCount = cktBlk->getDeviceNames(deviceNames);

    N_ERH_ErrorMgr::safeBarrier(pdsComm->comm());     // All procs call (5)

    pdsComm->maxAll(&byteCount, &maxByteCount, 1);
    ...
    {
      length = n_i->size();
      pdsComm->pack (&length, 1, &sendBuffer[0], byteCount, pos);
      pdsComm->pack (n_i->c_str(), length, &sendBuffer[0], byteCount, pos);
    }

    sendProc = pdsComm->procID()+1;
    recvProc = pdsComm->procID()-1;
    if (sendProc == numProcs) sendProc = 0;
    if (recvProc == -1) recvProc = numProcs-1;
    while (sendProc != pdsComm->procID())
    {
      pdsComm->iRecv( &bsize, 1, recvProc );
      pdsComm->send( &byteCount, 1, sendProc );
      pdsComm->waitAll();
      pdsComm->iRecv( &recvBuffer[0], bsize, recvProc );
      pdsComm->send( &sendBuffer[0], byteCount, sendProc );
      pdsComm->waitAll();
      deviceNames.resize(0);
      pos = 0;
      while (pos < bsize)
      {
        pdsComm->unpack(&recvBuffer[0], bsize, pos, &length, 1);
        deviceNames.push_back(std::string((&recvBuffer[0])+pos, length));
        pos += length;
      }

      cktBlk->checkDeviceNames(deviceNames);
      N_ERH_ErrorMgr::safeBarrier(pdsComm->comm());          // All procs call (5+N)
      if (++sendProc == numProcs) sendProc = 0;
      if (--recvProc == -1) recvProc = numProcs-1;
    }
  }
