...
//-----------------------------------------------------------------------------
struct FourierOptionsReg : public PkgOptionsReg
{
  FourierOptionsReg( FourierMgr &mgr )
    : fourierManager_(mgr)
  {}
...
//-----------------------------------------------------------------------------
bool FourierMgr::registerPkgOptionsMgr(PkgOptionsMgr &pkgOpt)
{
  pkgOpt.submitRegistration(
    "FOUR", netListFilename_, new FourierOptionsReg(*this));
  return true;
}
...
void
FourierMgr::fixupFourierParameters(Parallel::Machine comm, const Util::Op::BuilderManager &op_builder_manager) 
{
  Util::Op::makeOps(comm, op_builder_manager, depSolVarIterVector_.begin(), depSolVarIterVector_.end(), std::back_inserter(outputVars_));
...
bool FourierMgr::addFourierAnalysis(const Util::OptionBlock & fourierBlock)
{
  ...
  for (Util::ParameterList::const_iterator currentParamIt = fourierBlock.begin(); currentParamIt != const_cast<N_UTL_OptionBlock &>(fourierBlock).end(); ++currentParamIt)
  {
    std::string name = "";
...
// Creation Date : 03/10/2009
//-----------------------------------------------------------------------------
void FourierMgr::updateFourierData(Parallel::Machine comm, const double circuitTime, const N_LAS_Vector *solnVec)
{
  // Save the time.
  ...
  {
    double retVal = Util::Op::getValue(comm, *outputVars_[i], Util::Op::OpData(0, solnVec, 0, 0, 0, 0)).real();
