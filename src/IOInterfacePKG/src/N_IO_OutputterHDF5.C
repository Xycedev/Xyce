...
//-------------------------------------------------------------------------
// Filename       : $RCSfile: N_IO_OutputterHDF5.C,v $
//
...
bool OutputMgr::prepareHDF5Output(Parallel::Machine comm)
{
  ...
  {
#ifdef Xyce_PARALLEL_MPI
    H5Pset_fapl_mpio(hdf5PlistId_, comm_, MPI_INFO_NULL);
  ...
  const N_LAS_Vector &  solnVecPtr)
{
  ...
  if (!hdf5HeaderWritten_)
  {
    ...
    for (NodeNamePairMap::iterator nameIter = allNodes_.begin(), nameEnd = allNodes_.end(); nameIter != nameEnd ; ++nameIter)
    {
      int nodeNameLength = nameIter->first.size();
      globalMaxNodeNameLength = Xycemax(globalMaxNodeNameLength, nodeNameLength);
    }
    ...
    // fill up the arrays
    int index = 0;
    for (NodeNamePairMap::iterator nameIter = allNodes_.begin(), nameEnd = allNodes_.end(); nameIter != nameEnd ; ++nameIter, index++)
    {
