...
// Creation Date : 03/10/2009
//-----------------------------------------------------------------------------
Base::Base( const Util::OptionBlock & measureBlock)
  : name_(""),
    mode_(""),
    type_(""),
...
{
  makeOps(comm, op_builder_manager, depSolVarIterVector_.begin(), depSolVarIterVector_.end(), std::back_inserter(outputVars_));
...
//-----------------------------------------------------------------------------
void Base::updateOutputVars(Parallel::Machine comm, std::vector<double> & outputVarVec, const double circuitTime, const N_LAS_Vector *solnVec, 
...

{
  ...
  {
    outputVarVec[vecIndex] = getValue(comm, *(*it), Util::Op::OpData(vecIndex, solnVec, imaginaryVec, stateVec, storeVec, 0)).real();
    vecIndex++;
  }
}
...
// Creation Date : 03/10/2009
//-----------------------------------------------------------------------------
double Base::getOutputValue(Parallel::Machine comm, Util::Op::Operator *op, const N_LAS_Vector *solnVec, const N_LAS_Vector *stateVec, 
  const N_LAS_Vector * storeVec, const N_LAS_Vector *imaginaryVec )
{
  double retVal = getValue(comm, *op, Util::Op::OpData(0, solnVec, imaginaryVec, stateVec, storeVec, 0)).real();
  return retVal;
}
