...
//-----------------------------------------------------------------------------
// Filename      : $RCSfile: N_IO_MeasureRMS.C,v $
...
//-----------------------------------------------------------------------------
RMS::RMS( const Util::OptionBlock & measureBlock):
  Base(measureBlock),
  rootMeanSqureValue_(0.0),
  lastTimeValue_(0.0),
...
// Creation Date : 3/10/2009
//-----------------------------------------------------------------------------
void RMS::updateTran(Parallel::Machine comm, const double circuitTime, const N_LAS_Vector *solnVec, const N_LAS_Vector *stateVec, const N_LAS_Vector *storeVec)
{
  if( !calculationDone_ && withinFromToWindow( circuitTime ) )
  {
    ...
    if( initialized_  )
    {
      ...
      rootMeanSqureValue_ += (circuitTime - lastTimeValue_) * 0.25 *(outVarValues_[0] + lastSignalValue_)*(outVarValues_[0] + lastSignalValue_);
...
// Creation Date : 3/10/2009
//-----------------------------------------------------------------------------
void RMS::updateDC(Parallel::Machine comm, const std::vector<N_ANP_SweepParam> & dcParamsVec, const N_LAS_Vector *solnVec, const N_LAS_Vector *stateVec, const N_LAS_Vector *storeVec)
{
...
double RMS::getMeasureResult()
{
  if( initialized_ )
  {
    calculationResult_ =  sqrt(rootMeanSqureValue_ / totalAveragingWindow_);
  }
