...
//
// Revsion Date   : $Date: 2014/08/07 00:44:49 $
//
...
public:
  ...
  void incrementDeviceCount(std::string device = "");
  void incrementLinearDeviceCount();

  // Add a subcircuit name to the instance list. The instance list
  ...
  std::list<std::string>& getUsedContextList()
  {
    return usedContextList_;
  }
  ...
  void pruneContexts( std::list<std::string>& keepContexts );
...
private:
  ...
  std::list<std::string>         usedContextList_;
  std::map<std::string, int>     localDeviceCountMap_;
  std::map<std::string, NetlistLocation> instanceErrorInfo_; 
...
//----------------------------------------------------------------------------
inline void CircuitContext::incrementDeviceCount(std::string device)
{
