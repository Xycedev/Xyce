...
//-------------------------------------------------------------------------
// Filename       : $RCSfile: N_PDS_MPI.h,v $
//
// Purpose        : 
//                  
//                  
...
#ifndef XYCE_UTIL_MPI_h
#define XYCE_UTIL_MPI_h
...
template<class T>
void
AllGather(MPI_Comm mpi_comm, const T &src, std::vector<T> &dest)
{
  if (mpi_parallel_run(mpi_comm)) {
    int nproc = 1;
    MPI_Comm_size(mpi_comm, &nproc);
    ...
    if (MPI_Allgather(const_cast<T*>(&src), 1, Datatype<T>::type(),
