...
// ---------------------
// Revision Number: $Revision: 1.135.2.4 $
// Revision Date  : $Date: 2014/08/28 16:25:38 $
...
//-----------------------------------------------------------------------------
Transient::Transient( AnalysisManager &analysis_manager )
  : AnalysisBase(analysis_manager),
    Util::ListenerAutoSubscribe<StepEvent>(&analysis_manager),
  comm_(analysis_manager.getPDSComm().comm()),
...
//-----------------------------------------------------------------------------
bool Transient::setAnalysisParams(const Util::OptionBlock & paramsBlock)
{
  for (Util::ParameterList::const_iterator it = paramsBlock.getParams().begin(), end = paramsBlock.getParams().end(); it != end; ++it)
  {
    if ((*it).uTag()      == "TSTART")
    {
      tiaParams_.tStart = (*it).getImmutableValue<double>();
      tiaParams_.tStartGiven = true;
    }
    else if ((*it).uTag() == "TSTOP")
    {
      tiaParams_.finalTime   = (*it).getImmutableValue<double>();
    }
    else if ((*it).uTag() == "TSTEP")
    {
      tiaParams_.userSpecified_startingTimeStep = (*it).getImmutableValue<double>();
    }
    else if ((*it).uTag() == "NOOP" ||
             (*it).uTag() == "UIC")
    {
      tiaParams_.NOOP = 1;
    }
    else if ((*it).uTag() == "DTMAX")
    {
      tiaParams_.maxTimeStep = (*it).getImmutableValue<double>();
      tiaParams_.maxTimeStepGiven = true;
#ifdef Xyce_DEBUG_ANALYSIS
      if (tiaParams_.debugLevel > 0)
      {
        dout() << "setting maxTimeStep = " << tiaParams_.maxTimeStep << std::endl;
      }
#endif
    }
    else if ((*it).uTag() == "MAXTIMEEXPRESSION")
    {
      ...
      maxTimeStepExpressionAsString_ = (*it).stringValue();
    }
  }

  if (tiaParams_.finalTime <= tiaParams_.tStart || tiaParams_.finalTime <= 0.0 || tiaParams_.tStart < 0.0)
  {
    UserFatal(*this) << "Final time of " << tiaParams_.finalTime
                     << " is earlier or same as start time of " << tiaParams_.tStart << std::endl
  ...
  // control.
  if ( tiaParams_.userSpecified_startingTimeStep <= 0.0 )
  {
    tiaParams_.userSpecified_startingTimeStep = 1.0e-10;
  }
  ...
  if (tiaParams_.debugLevel > 0)
  {
    dout() << std::endl
           ...
           << "  starting time step = " << tiaParams_.userSpecified_startingTimeStep << std::endl
           << "  tStart (time of first output) = " << tiaParams_.tStart << std::endl;
  ...
  if( maxTimeStepExpressionGiven_ )
  {
    ...
    // max time step value
    maxTimeStepExpressionGiven_ = true;
    maxTimeStepExpressionRCPtr_ = rcp( new Util::ExpressionData( maxTimeStepExpressionAsString_) );
  }
  ...
  if (tiaParams_.historyTrackingDepth > 0 )
  {
    historyTrackingOn_ = true;
    queueSize_= tiaParams_.historyTrackingDepth;
    timeQueue_.set_size( queueSize_ );
    timeStepQueue_.set_size( queueSize_ );
    stepStatusQueue_.set_size( queueSize_ );
    estErrorOverTolQueue_.set_size( queueSize_ );
...
bool Transient::run()
{
  ...
  if (analysisManager_.getHBFlag() && !analysisManager_.getMPDEStartupFlag())
    resetForHB();
...
bool Transient::init()
{
  ...
  initialIntegrationMethod_ = TIAMethod_ONESTEP;
  if (tiaParams_.integrationMethod != TIAMethod_ONESTEP)
  {
    initialIntegrationMethod_ = tiaParams_.integrationMethod;
  }

  if (!tiaParams_.resume)
  {
    dcopFlag_ = !tiaParams_.NOOP;
    if(dcopFlag_)
    {
    ...
    {
      analysisManager_.setCurrentMode(Analysis::CURRENT_MODE_TRANSIENT);
    }

    doubleDCOPFlag_ = loader_.getDoubleDCOPFlag();
    doubleDCOPStep_ = tiaParams_.firstDCOPStep;

    if (analysisManager_.getRestartManager().isRestart())
    {
      loader_.getInitialQnorm(analysisManager_.getTIADataStore()->innerErrorInfoVec);

      workingIntgMethod_->createTimeIntegMethod(integrationMethod_);
      workingIntgMethod_->initialize();
      ...
      linearSystem_.updateExternValsSolnVector(analysisManager_.getTIADataStore()->nextSolutionPtr);
      linearSystem_.updateExternValsSolnVector(analysisManager_.getTIADataStore()->currSolutionPtr);
      linearSystem_.updateExternValsStateVector(analysisManager_.getTIADataStore()->nextStatePtr);
      linearSystem_.updateExternValsStateVector(analysisManager_.getTIADataStore()->currStatePtr);
      linearSystem_.updateExternValsStoreVector(analysisManager_.getTIADataStore()->nextStorePtr);
      linearSystem_.updateExternValsStoreVector(analysisManager_.getTIADataStore()->currStorePtr);
#endif
      ...
      nonlinearSolverManager_.setAnalysisMode(anpAnalysisModeToNLS(ANP_MODE_TRANSIENT));
    }
    else
    {
      if (dcopFlag_)
      ...
      if (!analysisManager_.getMPDEFlag() && !analysisManager_.getMPDEStartupFlag())
      {
        ...
        inputOPFlag_ = outputManagerAdapter_.setupInitialConditions
          ( *analysisManager_.getTIADataStore()->nextSolutionPtr, *analysisManager_.getTIADataStore()->flagSolutionPtr);
      }

      if (!dcopFlag_ && !analysisManager_.getMPDEFlag())
      {
      ...
      if (!dcopFlag_ && !analysisManager_.getMPDEFlag() && sensFlag_ && !tiaParams_.resume)
      {
        nonlinearSolverManager_.icSensitivity (objectiveVec_, 
                                  dOdpVec_, dOdpAdjVec_, scaled_dOdpVec_, scaled_dOdpAdjVec_);

        analysisManager_.getTIADataStore()->setConstantSensitivityHistory();
      }

      if (!dcopFlag_ && !tiaParams_.resume)
      {
  ...
  {
    if (analysisManager_.getCurrentMode() == CURRENT_MODE_TRANSIENT)
    {
    ...
    // catch that.
    if (workingIntgMethod_->getIntegMethodPtr() == NULL)
    {
      integrationMethod_ = tiaParams_.integrationMethod;
      workingIntgMethod_->createTimeIntegMethod(integrationMethod_);
    }
  ...
  if (!tiaParams_.resume)
  {
    ...
    if( maxTimeStepExpressionGiven_ )
    {
      maxTimeStepExpressionRCPtr_->setup(comm_, outputManagerAdapter_.getOutputManager());

      suggestedMaxTime = maxTimeStepExpressionRCPtr_->evaluate(comm_, outputManagerAdapter_.getOutputManager().getCircuitTime(),
        analysisManager_.getTIADataStore()->currSolutionPtr, analysisManager_.getTIADataStore()->currStatePtr, analysisManager_.getTIADataStore()->currStorePtr);
    }
    stepErrorControl_->updateMaxTimeStep( suggestedMaxTime );
    stepErrorControl_->updateMinTimeStep();
...
bool Transient::loopProcess()
{
  ...
  while (!(stepErrorControl_->isFinished()))
  {
    ...
    stepErrorControl_->updateStopTime();
    ...
    if (tiaParams_.debugLevel > 0)
    {
      ...
      dout() << "stepErrorControl_->stepAttemptStatus = " << stepErrorControl_->stepAttemptStatus << std::endl;
    }
#endif

    if (beginningIntegration &&
        stepErrorControl_->stepAttemptStatus)
    {
      // ------------------------------------------------------------------------
...
#ifdef Xyce_VERBOSE_TIME
    if (!dcopFlag_)
      stepErrorControl_->outputTimeInfo(lout());
#endif
    ...
    static_cast<Xyce::Util::Notifier<AnalysisEvent> &>(analysisManager_).publish(AnalysisEvent(AnalysisEvent::STEP_STARTED, AnalysisEvent::TRAN, stepErrorControl_->nextTime, stepErrorControl_->getNumberOfSteps()));
    ...
    // ------------------------------------------------------------------------
    if (dcopFlag_ && stepErrorControl_->stepAttemptStatus)
    {
      processSuccessfulDCOP();
    }
    else if (dcopFlag_ && !stepErrorControl_->stepAttemptStatus)
    {
      processFailedDCOP();
      bsuccess = false;
      break;
    ...
    else
    {
      ...
      else if( (tiaParams_.passNLStall == true) &&
               !(stepErrorControl_->stepAttemptStatus)  &&
               (stepErrorControl_->currentTimeStep < (4*stepErrorControl_->minTimeStep)) )
      {
        ...
        if( stepErrorControl_->newtonConvergenceStatus == -3)
        {
          {
        ...
        // time step, then calls this failure a pass
        if( stepErrorControl_->newtonConvergenceStatus == -2)
        {
          {
        ...
        //
        // if( stepErrorControl_->newtonConvergenceStatus == -1)
        // {
    ...
    if (tiaParams_.debugLevel > 0)
    {
      ...
      dout() << "   minTimeStep = " << stepErrorControl_->minTimeStep << std::endl;
      dout() << "   final time = " << tiaParams_.finalTime << std::endl;
      dout() << "   pause time = " << analysisManager_.getPauseTime() << std::endl;
      dout() << "   initial time = " << stepErrorControl_->initialTime << std::endl;
      dout() << "   current time = " << stepErrorControl_->currentTime << std::endl;
      if (analysisManager_.getPauseTime() == stepErrorControl_->currentTime)
      {
      ...
      else
      {
        dout() << "     difference between current and pause times is "
          << analysisManager_.getPauseTime() - stepErrorControl_->currentTime << std::endl;
      }
      if (analysisManager_.getPauseTime() == stepErrorControl_->initialTime)
      {
    ...
    if (tiaParams_.exitTime != 0.0 &&
        stepErrorControl_->currentTime > tiaParams_.exitTime)
    {
    ...
    if (tiaParams_.exitStep != -1 &&
          static_cast<int>(stepNumber) == tiaParams_.exitStep)
    {
  ...
  endTRANtime = analysisManager_.getXyceTranTimer().elapsedTime();
...
void Transient::preStepDetails(double maxTimeStepFromHabanero)
{
  ...
  {
    double currentTimeStep = Xycemin(maxTimeStepFromHabanero, stepErrorControl_->currentTimeStep);
    stepErrorControl_->setTimeStep(currentTimeStep);
  }
...
bool Transient::finalizeStep ()
{
  ...
  if (tiaParams_.debugLevel > 0)
  {
    ...
    {
      dout() << "     difference between current and pause times is " << analysisManager_.getPauseTime() - stepErrorControl_->currentTime << std::endl;
    }
    if (analysisManager_.getPauseTime() == stepErrorControl_->initialTime)
    {
      dout() << "    Pause time and initial time equal " << std::endl;
...
bool Transient::processSuccessfulDCOP()
{
  ...
  if( analysisManager_.testRestartSaveTime() )
  {
    if (DEBUG_RESTART)
      dout() << "\n " << analysisManager_.getCommandLine().getArgumentValue("netlist")
                   << "  Calling dumpRestartData" << std::endl;

    analysisManager_.getRestartManager().dumpRestartData( stepErrorControl_->currentTime );
  ...
  nonlinearSolverManager_.allocateTranSolver();
  stepErrorControl_->previousCallStepSuccessful = true;
...
{
  static_cast<Xyce::Util::Notifier<AnalysisEvent> &>(analysisManager_).publish(AnalysisEvent(AnalysisEvent::STEP_SUCCESSFUL, AnalysisEvent::TRAN, stepErrorControl_->nextTime, stepErrorControl_->getNumberOfSteps()));
  ...
  if( historyTrackingOn_ )
  {
    ...
    timeQueue_.push_back( stepErrorControl_->currentTime );
    timeStepQueue_.push_back( stepErrorControl_->currentTimeStep );
    stepStatusQueue_.push_back( 1 );
    estErrorOverTolQueue_.push_back( stepErrorControl_->estOverTol_);
    nonlinearSolverStatusQueue_.push_back( stepErrorControl_->newtonConvergenceStatus );
    nonlinearSolverNumIterationsQueue_.push_back( stepErrorControl_->nIterations);
    nonlinearSolverMaxNormQueue_.push_back( nonlinearSolverManager_.getMaxNormF() );
    nonlinearSolverMaxNormIndexQueue_.push_back( nonlinearSolverManager_.getMaxNormFindex () );
  }
  ...
  if (tiaParams_.debugLevel > 0)
  {
    ...
    analysisManager_.getTIADataStore()->nextSolutionPtr->printPetraObject(dout());
    dout() << std::endl;
  }
#endif
  ...
  double currentTime = stepErrorControl_->currentTime;
  double suggestedMaxTime=0.0;
  if( maxTimeStepExpressionGiven_ )
  {
    suggestedMaxTime = maxTimeStepExpressionRCPtr_->evaluate(comm_, outputManagerAdapter_.getOutputManager().getCircuitTime(),
  ...
  workingIntgMethod_->completeStep();

#ifdef Xyce_VERBOSE_TIME
  if (tiaParams_.errorAnalysisOption == 1)
  {
    dout().precision(15);
    dout() << "ERROROPTION=1: TimeStepLimitedbyBP = " << tiaParams_.TimeStepLimitedbyBP << "\n" << std::endl;
    dout() << "ERROROPTION=1: NL Its =  " << stepErrorControl_->nIterations << "\n" << std::endl;
    dout() << "ERROROPTION=1: New DeltaT = " << stepErrorControl_->currentTimeStep << "\n" << std::endl;
  }
#endif // Xyce_VERBOSE_TIME
  ...
  if (stepErrorControl_->numberSuccessiveFailures < 0)
    stepErrorControl_->numberSuccessiveFailures = 0;
  ...
  double bpTol = analysisManager_.getBreakpointTol();

  if (tiaParams_.bpEnable)
  {
    double timeDiff1 = stepErrorControl_->currentTime - stepErrorControl_->stopTime;
    double timeDiff2 = stepErrorControl_->currentTime - tiaParams_.finalTime;
  ...
  if (tiaParams_.errorAnalysisOptionResetIter > 1)
  {
    tiaParams_.errorAnalysisOptionResetIter--;
  }
  else if(tiaParams_.errorAnalysisOptionResetIter == 1)
  {
    tiaParams_.errorAnalysisOption = 0;
    tiaParams_.errorAnalysisOptionResetIter = 0;
  }

  if (tiaParams_.saveTimeStepsFlag)
  {
    analysisManager_.getTIADataStore()->timeSteps.push_back(currentTime);
    analysisManager_.getTIADataStore()->timeStepsBreakpointFlag.push_back(beginningIntegration);
    N_LAS_Vector * aVecPtr = new N_LAS_Vector( *(analysisManager_.getTIADataStore()->currSolutionPtr) );
    analysisManager_.getTIADataStore()->fastTimeSolutionVec.push_back( aVecPtr );
    aVecPtr = new N_LAS_Vector( *(analysisManager_.getTIADataStore()->currStatePtr) );
    analysisManager_.getTIADataStore()->fastTimeStateVec.push_back( aVecPtr );
    aVecPtr = new N_LAS_Vector( *(analysisManager_.getTIADataStore()->daeQVectorPtr) );
    analysisManager_.getTIADataStore()->fastTimeQVec.push_back( aVecPtr );
    aVecPtr = new N_LAS_Vector( *(analysisManager_.getTIADataStore()->currStorePtr) );
    analysisManager_.getTIADataStore()->fastTimeStoreVec.push_back( aVecPtr );
  }
...
bool Transient::processFailedStep()
{
  ...
  static_cast<Xyce::Util::Notifier<AnalysisEvent> &>(analysisManager_).publish(AnalysisEvent(AnalysisEvent::STEP_FAILED, AnalysisEvent::TRAN, stepErrorControl_->nextTime, stepErrorControl_->getNumberOfSteps()));
  ...
  double estOverTol = stepErrorControl_->getEstOverTol();
  if( (stepNumberAtMinEstErrorOverTol < 0) || ( estOverTol < minEstErrorOverTol ) )
  {
    ...
    minEstErrorOverTol = stepErrorControl_->getEstOverTol();
    timeStepAtMinEstErrorOverTol = stepErrorControl_->currentTimeStep;
  }
  ...
  if (stepErrorControl_->currentTimeStep <= stepErrorControl_->minTimeStep)
  {
#ifdef Xyce_DEBUG_ANALYSIS
    if (tiaParams_.debugLevel > 0)
    {
      dout() << "currTimeStep: " << stepErrorControl_->currentTimeStep << std::endl;
      dout() << "minTimeStep:  " << stepErrorControl_->minTimeStep << std::endl;
    }
#endif
  ...
  if (tiaParams_.exitStep != -1 &&
          static_cast<int>(stats_.successStepsThisParameter_) == (tiaParams_.exitStep - 1))
  {
    outputQueuedData();
    lout() << "Exit Step.  Exiting transient loop\n" << std::endl;
    bsuccess = false;
...
bool Transient::processFailedDCOP()
{
  ...
  stepErrorControl_->numberSuccessiveFailures++;
...
bool Transient::finish()
{
  ...
  if (tiaParams_.saveTimeStepsFlag && analysisManager_.getHBFlag())
  {
    N_TIA_DataStore *dsPtr_ = analysisManager_.getTIADataStore();
    dsPtr_->timeSteps.push_back(stepErrorControl_->currentTime);
...
bool Transient::handlePredictor()
{
  ...
  if (tiaParams_.debugLevel > 1)
  {
    ...
    analysisManager_.getTIADataStore()->outputPredictedSolution(Xyce::dout());
    analysisManager_.getTIADataStore()->outputPredictedDerivative(Xyce::dout());
  }
#endif
#endif
...
bool Transient::resetForHB()
{
  nonlinearSolverManager_.resetAll(Nonlinear::DC_OP);
  stepErrorControl_->resetAll();
  analysisManager_.getTIADataStore()->resetAll();
  dcopFlag_ = false;
  analysisManager_.setNextOutputTime(0.0);
...
bool Transient::finalVerboseOutput()
{
  ...
  {
    double time;
    if (analysisManager_.getStartTranTime() > startDCOPtime)
    {
      time = analysisManager_.getStartTranTime() - startDCOPtime;
    }
    else
    {
  ...
  if (analysisManager_.getAnalysisMode() == ANP_MODE_TRANSIENT &&
      endTRANtime >= analysisManager_.getStartTranTime())
  {
    lout() << " ***** Transient Stepping time: " << endTRANtime-analysisManager_.getStartTranTime() << " seconds.  Breakdown follows:" << std::endl;
...
void Transient::outputQueuedData()
{
  if( historyTrackingOn_ )
  {
    ...
    const Xyce::NodeNamePairMap & nodeNames = outputManagerAdapter_.getAllNodes();
    int nodeNameSize = nodeNames.size();
    nameVec_.resize(nodeNameSize+1,"gnd");
    Xyce::NodeNamePairMap::const_iterator mapI, mapEnd;
    ...
    N_NLS_ReturnCodes nlReturnCodes = nonlinearSolverManager_.getReturnCodes();
...
#ifdef Xyce_DEBUG_ANALYSIS
  dout() << "Transient::takeAnIntegrationStep_:  newtonConvergenceStatus = " <<  stepErrorControl_->newtonConvergenceStatus << std::endl;
#endif

  analysisManager_.getTIADataStore()->stepLinearCombo ();
  gatherStepStatistics_ ();
  stepErrorControl_->nIterations = nonlinearSolverManager_.getNumIterations();
...
bool Transient::retakeAndAcceptTimeStep( double aTimeStep )
{
  ...
  // set time step
  stepErrorControl_->currentTimeStep = timeStepAtMinEstErrorOverTol;

  // take step
  takeAnIntegrationStep_();

  // can't accept step if the non-linear solver failed
  if(stepErrorControl_->nIterations==0)
  {
    lout() << "Time step too small near step number: " <<  stepNumber << "  Exiting transient loop.\n" << std::endl;
    bsuccess = false;
  }
...
void Transient::printStepHeader(std::ostream &os)
{
  if (VERBOSE_TIME) {
    ...
    else
    {
      if (beginningIntegration)
      {
        if (stepErrorControl_->currentTime == tiaParams_.initialTime)
        {
...
void Transient::printProgress(std::ostream &os)
{
  if (analysisManager_.getProgressFlag())
  {
    ...
    {
      analysisManager_.setStartTranTime(analysisManager_.getXyceTranTimer().elapsedTime());
    ...
    {
      if (startSimTime == -1.0)
        startSimTime = stepErrorControl_->currentTime;

      double diff1 = fabs(stepErrorControl_->currentTime - tiaParams_.initialTime);
      ...
      if (fabs(percentComplete - oldPercentComplete) > 1.0)
      {
        ...
        aveCPUTimePerStep = analysisManager_.getXyceTranTimer().elapsedTime() /
          (stats_.successfulStepsTaken_ + 1);
        aveSimTimePerStep = (stepErrorControl_->currentTime - startSimTime) /
          (stats_.successfulStepsTaken_ + 1);

        if (aveSimTimePerStep > Util::MachineDependentParams::MachineEpsilon())
          estCompletionTime = aveCPUTimePerStep *
            fabs(tiaParams_.finalTime - stepErrorControl_->currentTime) /
            aveSimTimePerStep;
...
void Transient::noopOutputs()
{
  if( testOutputTime_() )
  {
    if ( !firstDoubleDCOPStep_ () )
    {
      ...
      if (analysisManager_.getMPDEManager() && analysisManager_.getMPDEManager()->getMPDEFlag() && !analysisManager_.getMPDEManager()->getMPDEIcFlag())
      {
        // output the operating point if we can in MPDE mode
        outputManagerAdapter_.outputMPDE(stepErrorControl_->currentTime, analysisManager_.getMPDEManager()->getFastTimePoints(), *analysisManager_.getTIADataStore()->nextSolutionPtr);
      }
    }
    updateOutputTime_(stepErrorControl_->currentTime);
  }
}
...
void Transient::tranopOutputs ()
{
...
#endif
      outputManagerAdapter_.tranOutput(stepErrorControl_->currentTime, 
          *analysisManager_.getTIADataStore()->currSolutionPtr, 
          *analysisManager_.getTIADataStore()->currStatePtr, 
          *analysisManager_.getTIADataStore()->currStorePtr,
          objectiveVec_, 
  ...
  if ( analysisManager_.testDCOPOutputTime_() || analysisManager_.testSaveOutputTime_() )
  {
...
void Transient::tranStepOutputs ()
{
  ...
  bool doNotInterpolate=
      (beginningIntegration && tiaParams_.tStartGiven && firstTranOutput_);
  ...
  if (testOutputTime_())
  {
    if (analysisManager_.getMPDEManager() && analysisManager_.getMPDEManager()->blockAnalysisFlag())
    {
#ifdef Xyce_DEBUG_ANALYSIS
      if (tiaParams_.debugLevel > 0)
      {
      ...
      if (analysisManager_.getMPDEManager()->getMPDEFlag()==true && tiaParams_.outputInterpMPDE)
      {
        if (analysisManager_.getMPDEManager()->getWaMPDEFlag()==false)
        {
          std::vector<double> fastTimes = analysisManager_.getMPDEManager()->getFastTimePoints();
          workingIntgMethod_->printMPDEOutputSolution(
                  outputManagerAdapter_, stepErrorControl_->currentTime,
                  analysisManager_.getTIADataStore()->currSolutionPtr,
                  fastTimes );
        }
        else
        {
          std::vector<double> fastTimes = analysisManager_.getMPDEManager()->getFastTimePoints();
          int phiGID = analysisManager_.getMPDEManager()->getPhiGID();
          workingIntgMethod_->printWaMPDEOutputSolution(
                  outputManagerAdapter_, stepErrorControl_->currentTime,
                  analysisManager_.getTIADataStore()->currSolutionPtr,
                  fastTimes, phiGID );
        }
      ...
      else
      {
        ...
        computeOutputInterpolationTimes_(stepErrorControl_->currentTime);
        workingIntgMethod_->printOutputSolution(
              outputManagerAdapter_, stepErrorControl_->currentTime,
              analysisManager_.getTIADataStore()->currSolutionPtr,
              doNotInterpolate,
  ...
  // SAVE output:
  if ( analysisManager_.testSaveOutputTime_() )
  {
    workingIntgMethod_->saveOutputSolution(
                outputManagerAdapter_,
                analysisManager_.getTIADataStore()->currSolutionPtr,
                analysisManager_.getSaveTime(), doNotInterpolate);
  }
}
...
bool Transient::testOutputTime_()
{
  ...
  if (tiaParams_.debugLevel > 0)
  {
    ...
    dout() << "Transient::testOutputTime.  secPtr_->currentTime = " << stepErrorControl_->currentTime << std::endl;
    dout() << "Transient::testOutputTime.  tStart      = " << tiaParams_.tStart << std::endl;
    dout() << "Transient::testOutputTime.  nextOutputTime_ = " << analysisManager_.getNextOutputTime() << std::endl;
    for (int i=0;i<analysisManager_.getOutputIntervals().size();++i)
    {
      dout() << "outputIntervals_["<<i<<"].first = " << analysisManager_.getOutputIntervals()[i].first;
      dout() << std::endl;
    }
    dout() << std::endl;
  }
  ...
  if (stepErrorControl_->currentTime < tiaParams_.tStart)
  {
    flag = false;
  }
  else if (stepErrorControl_->currentTime >= tiaParams_.finalTime)
  {
    flag = true;
  }
  else if (analysisManager_.getInitialOutputInterval() == 0.0)
  {
    flag = true;
  }
  else if (stepErrorControl_->currentTime < analysisManager_.getNextOutputTime())
  {
    flag = false;
  }
  else
...
void Transient::updateOutputTime_(double currTime)
{
  ...
  // options
  if (analysisManager_.outputIntervalSpecified())
  {
    if (analysisManager_.getOutputIntervals().empty())
    {
      // Only an initial interval was specified
      while (analysisManager_.getNextOutputTime() <= currTime)
        analysisManager_.setNextOutputTime(analysisManager_.getNextOutputTime() + analysisManager_.getInitialOutputInterval());
    }
    else if (currTime < analysisManager_.getOutputIntervals()[0].first)
    {
      // Multiple intervals, but we're still in the first one
      while (analysisManager_.getNextOutputTime() <= currTime)
        analysisManager_.setNextOutputTime(analysisManager_.getNextOutputTime() + analysisManager_.getInitialOutputInterval());
      if (analysisManager_.getNextOutputTime() > analysisManager_.getOutputIntervals()[0].first)
        analysisManager_.setNextOutputTime(analysisManager_.getOutputIntervals()[0].first);
    }
    else
    {
      // Multiple intervals specified, we're past the first one
      std::pair<double, double> currInterval, nextInterval;
      int size = analysisManager_.getOutputIntervals().size();
      for (int i = 0; i < size; ++i)
        if (analysisManager_.getOutputIntervals()[i].first <= currTime)
        {
          currInterval = analysisManager_.getOutputIntervals()[i];
          if ((i+1) < static_cast<int>(analysisManager_.getOutputIntervals().size()))
            nextInterval = analysisManager_.getOutputIntervals()[i+1];
        }
      int step = static_cast<int> ((currTime-currInterval.first) /
                                   currInterval.second);
      analysisManager_.setNextOutputTime(currInterval.first + (step+1)*currInterval.second);
      if (nextInterval.first && (nextInterval.first!=currInterval.first)
          && (analysisManager_.getNextOutputTime()>=nextInterval.first))
        analysisManager_.setNextOutputTime(nextInterval.first);
    }

    if (analysisManager_.getNextOutputTime() >= tiaParams_.finalTime)
      analysisManager_.setNextOutputTime(tiaParams_.finalTime);
  }
}
...
void Transient::computeOutputInterpolationTimes_(double currTime)
{
  ...
  if (analysisManager_.outputIntervalSpecified())
  {
    if (analysisManager_.getOutputIntervals().empty() || currTime <= analysisManager_.getOutputIntervals()[0].first)
    {
      // we're either in the first interval, or there only *is* one interval
      t=analysisManager_.getNextOutputTime();
      while (t < currTime)
      {
        outputInterpolationTimes_.push_back(t);
        t += analysisManager_.getInitialOutputInterval();
      }

      if( (t - currTime) <= 100 * Util::MachineDependentParams::MachinePrecision() )
      {
        outputInterpolationTimes_.push_back(currTime);
      }

      if( (t - tiaParams_.finalTime) > 100 * Util::MachineDependentParams::MachinePrecision() )
      {
        // tack on finalTime or we'll miss it
        outputInterpolationTimes_.push_back( tiaParams_.finalTime );
      }
    }
    else // we're out of the first interval, and there is more than one
    {
      ...
      lastInt=analysisManager_.getOutputIntervals().size()-1;

      // find which interval nextOutputTime_ is in
      for (outInt=0;
           outInt<lastInt&&analysisManager_.getOutputIntervals()[outInt+1].first<=analysisManager_.getNextOutputTime();
           ++outInt) ;
      ...
      {
        outputInterpolationTimes_.push_back(t);
        t += analysisManager_.getOutputIntervals()[outInt].second;
        if (outInt != lastInt && t >= analysisManager_.getOutputIntervals()[outInt+1].first)
        {
          ++outInt;
          t = analysisManager_.getOutputIntervals()[outInt].first;
        }
      }
    }
  }
