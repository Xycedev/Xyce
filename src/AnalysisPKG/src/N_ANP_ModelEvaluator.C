...
// Revision Date  : $Date: 2014/08/07 21:23:14 $
...
void ModelEvaluator::initialize(int iargs, char* cargs[])
{
...
std::vector<std::string> ModelEvaluator::getVariableNames()
{
  return xycePtr_->getVariableNames();
}


void ModelEvaluator::setupInOutArgs_()
{
  Np_ = 2;
  Ng_ = 3;
...
void ModelEvaluator::setupMapsAndGraphs_()
{
  ...
  int BlockSize = x_map_->numGlobalEntities();
  blockMap_ = createBlockParMap( Size, *x_map_ );
  ...
  {
    BaseRow = x_map_->localToGlobalIndex(j);
...
{
  return rcp( blockMap_->petraMap(), false );
}
...
RCP<const Epetra_Map> ModelEvaluator::get_p_map(int p) const
{
...
RCP<const Epetra_Map> ModelEvaluator::get_g_map(int p) const
{
...
RCP<const Epetra_Map> ModelEvaluator::get_small_x_map() const
{
...
void ModelEvaluator::evalModel( const InArgs& inArgs, const OutArgs& outArgs ) const
{
  ...
  RCP<N_LAS_BlockVector> xyce_x    = convertEpetraToNLASBlockVectorView(x_in,x_map_);
  RCP<N_LAS_BlockVector> xyce_xdot = convertEpetraToNLASBlockVectorView(xdot_in,x_map_);
  ...
  if (!Teuchos::is_null(s_out)) {
    ...
    RCP<N_LAS_BlockVector> xyce_f        = convertEpetraToNLASBlockVectorView(f_out,x_map_);
    ...
    // Eval model through AnalysisManager
    ...
    xycePtr_->evalTransientModel(
      ...
      & store,
      & store,
      & store,   // this should e storLeadCurrQ !
      & f_1_ref, // f_1 = q
      & f_0_ref, // f_0 = f
      & f_0_ref, // THIS WILL NOT WORK.  It needs to be the B-vector.
      & voltLimF_ref,
      & voltLimQ_ref,
      &* dQdx_gnd_matrix_,
      &* dFdx_gnd_matrix_
...

    {
      ...
      // Copy all data from blocks to global matrix if necessary.
      bMat->assembleGlobalMatrix();
    }
  }
}
...
RCP<Epetra_Operator> ModelEvaluator::create_W() const
{
  ...
  int MaxGID = x_map_->maxGlobalEntity();
  int offset=1;
  while ( offset <= MaxGID ) offset *= 10;

  RCP<Epetra_CrsGraph> blockGraph = createBlockGraph( offset, Cols, *blockMap_, *dFdx_graph_with_diagonal_ );

  // Create the N_LAS_BlockMatrix:
  RCP<N_LAS_BlockMatrix> bMat = rcp( new N_LAS_BlockMatrix( Size, offset, Cols, *blockGraph, *dFdx_graph_with_diagonal_ ) );
  bMat.release(); // will leak memory now.
 
 // Get the underlying Epetra_CrsMatrix out of this
  ...
  // NOTE:  Since the blocks of bMat are NOT views of the underlying Epetra_CrsMatrix, one needs to be careful.
...
bool ModelEvaluator::isInitialized() const 
{ 
