...
// Creation Date : 5/11
//-----------------------------------------------------------------------------
MOR::MOR( AnalysisManager &analysis_manager ) :
...
bool MOR::setAnalysisParams(const N_UTL_OptionBlock & paramsBlock)
{
  ...
  for (it_tp = first; it_tp != last; ++it_tp)
  {
    if (it_tp->uTag()      == "SIZE")
    {
      tiaParams_.ROMsize = it_tp->getImmutableValue<int>();
    }
...
//  getDebugLevel() = 1;
  ...
  if (analysisManager_.getDebugLevel() > 0)
  {
    dout() << std::endl
           ...
           << " size = " << tiaParams_.ROMsize << std::endl;
  }
#endif
...
bool MOR::init()
{
  ...
  s0_ =  2.0 * M_PI * tiaParams_.morExpPoint;
  ...
  inputOPFlag_ =
    outputManagerAdapter_.setupInitialConditions( *(analysisManager_.getTIADataStore()->nextSolutionPtr),
  ...
  loader_.getBMatrixEntriesforMOR(tempVec, bMatPosEntriesVec_);

  // Determine how many global ports their are performing a sumAll()
  int hsize = tempVec.size();
  N_PDS_Comm * pdsComm = (linearSystem_.getPDSManager())->getPDSComm();
  ...
  for (int i=0; i<hsize; ++i)
  {
    ...
    char type1;
    analysisManager_.getTopology().getNodeSVarGIDs(NodeID(portList_[i],_VNODE), svGIDList1, dummyList, type1);
  ...
  N_PDS_Manager &pdsMgrPtr_ = *linearSystem_.getPDSManager();
  ...
  analysisManager_.getTIADataStore()->daeBVectorPtr->putScalar(0.0);
...
bool MOR::reduceSystem()
{
  ...
  // the tiaParams_.morMethod option, which would be best implemented using a factory design.
  ...
  N_PDS_ParMap &BaseMap_ = *pdsMgrPtr_.getParallelMap( "SOLUTION" );
  ...
  int kblock = 0;
  if (tiaParams_.ROMsize > RPtr_->globalLength())
  {
  ...
  {
    kblock = (int)(tiaParams_.ROMsize / numPorts_);
  }
...
//  W->Print(Xyce::dout());
  ...
  // ---------------------------------------------------------------------
  if (tiaParams_.morSparsificationType)
  {
  ...
  int scaleType = tiaParams_.morScaleType;

  if ( scaleType == 1 )
  {
    for (int i=0; i<k; ++i)
    {
      Xhatscale( i, 0 ) = tiaParams_.morScaleFactor;
    }
  }
...
if ( scaleType == 2 || scaleType == 3  || scaleType ==4)
  {
    ...
    if ( scaleType == 2 )
      Xmag.PutScalar( tiaParams_.morScaleFactor );
...
//    Xhatscale.print(Xyce::dout());
    for (int i=0; i<k; ++i)
    {
      ...
      if ( scaleType == 2 )
      {
        if ( fabs(Xhatscale( i, 0 )) >  tiaParams_.morScaleFactor)
        {
          Xhatscale( i, 0 ) = tiaParams_.morScaleFactor / fabs( Xhatscale( i, 0 ) );
        }
        else
        {
          Xhatscale( i, 0 ) =  tiaParams_.morScaleFactor1 / fabs( Xhatscale( i, 0 ) );
        }
      }
...
//  W->Print(Xyce::dout());
  ...
  else
  {
    if ( scaleType <= 1 )
    {
      redCPtr_->scale(1.0/tiaParams_.morScaleFactor);
      redGPtr_->scale(1.0/tiaParams_.morScaleFactor);

      redL_.scale(1.0/tiaParams_.morScaleFactor);
    }
    else
    {
  ...
  if (tiaParams_.morSaveRedSys)
  {
    // If sparsification was used, write out the N_LAS_Matrix object instead of the dense object.
    if (tiaParams_.morSparsificationType)
      outputManagerAdapter_.outputROM ( *redGPtr_, *redCPtr_, redB_, redL_ );
    else
      outputManagerAdapter_.outputROM ( redG_, redC_, redB_, redL_ );  // L = B'
  }
...
bool MOR::createOrigLinearSystem_()
{
  ...
  N_PDS_ParMap &BaseMap_ = *pdsMgrPtr_.getParallelMap("SOLUTION");
  N_PDS_ParMap &oBaseMap_ = *pdsMgrPtr_.getParallelMap("SOLUTION_OVERLAP_GND");
  Epetra_CrsGraph &BaseFullGraph_ = *pdsMgrPtr_.getMatrixGraph("JACOBIAN");
  ...
  REFBPtr_ = rcp ( new N_LAS_BlockVector ( numBlocks, blockMaps[0], rcp(&BaseMap_, false) ) );
  ...
  REFXPtr_ = rcp ( new N_LAS_BlockVector ( numBlocks, blockMaps[0], rcp(&BaseMap_, false) ) );
...
{
  double fStart = tiaParams_.morCompFStart;
...
int MOR::setupSweepParam_()
{
  int fCount = 0;
  double fStart = tiaParams_.morCompFStart;
  double fStop = tiaParams_.morCompFStop;
  ...
  if (tiaParams_.morCompType=="LIN")
  {
    if ( tiaParams_.morCompNP == 1)
      fStep_ = 0;
    else
      fStep_  = (fStop - fStart)/(tiaParams_.morCompNP - 1);

    fCount = tiaParams_.morCompNP;
#ifdef Xyce_DEBUG_ANALYSIS
    if (analysisManager_.getDebugLevel() > 0)
    {
  ...
  else if(tiaParams_.morCompType=="DEC")
  {
    stepMult_ = pow(10,(1/(double)tiaParams_.morCompNP));
    fCount   = (int)(floor(fabs(log10(fStart) - log10(fStop)) * tiaParams_.morCompNP + 1));
#ifdef Xyce_DEBUG_ANALYSIS
    if (analysisManager_.getDebugLevel() > 0)
    {
  ...
  else if(tiaParams_.morCompType=="OCT")
  {
    stepMult_ = pow(2,1/(double)(tiaParams_.morCompNP));
    ...
    double ln2=log(2.0);
    fCount   = floor(fabs(log(fStart) - log(fStop))/ln2 * tiaParams_.morCompNP + 1);
#ifdef Xyce_DEBUG_ANALYSIS
    if (analysisManager_.getDebugLevel() > 0)
    {
      Xyce::dout() << "stepMult_ = " << stepMult_  << std::endl;
