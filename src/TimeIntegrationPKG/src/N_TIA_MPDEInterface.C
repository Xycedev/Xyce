...
// Revision Date  : $Date: 2014/08/07 21:23:16 $
...
bool N_TIA_MPDEInterface::runDCOP()
{
  if( !tiaParams_.NOOP )
  {
    ...
    anaManagerPtr_->setAnalysisMode(Xyce::Analysis::ANP_MODE_DC_SWEEP);
...
{
  anaManagerPtr_->setAnalysisMode(Xyce::Analysis::ANP_MODE_TRANSIENT);
...
// Creation Date : 10/22/07
//-----------------------------------------------------------------------------
void N_TIA_MPDEInterface::stepSuccess(Xyce::Analysis::CurrentMode analysisType)
{
...
bool N_TIA_MPDEInterface::reInitialize()
{
  ...
  anaManagerPtr_->getWorkingIntegrationMethod().initialize();
