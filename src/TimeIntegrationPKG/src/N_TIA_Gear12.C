...
// ---------------------
//
// Revision Number: $Revision: 1.34.2.6 $
//
// Revision Date  : $Date: 2014/08/29 22:44:16 $
//
...
//-----------------------------------------------------------------------------
// Function      : N_TIA_Gear12::obtainSensitivityPredictors
// Purpose       : Calculate predictor 
// Special Notes : 
// Scope         : public
...
void N_TIA_Gear12::obtainSensitivityPredictors()
{
  ...
  for (int ip=0; ip<numParams;++ip)
  {
    ...
    // evaluate predictor
    dXdpn0Ptr->putScalar(0.0);
    dqdpn0Ptr->putScalar(0.0);

    for (int i=0;i<=sec.currentOrder_;++i)
    {
      dXdpn0Ptr->linearCombo(sec.beta_[i],*(dXdpHistory[i]),1.0,*ds.xn0Ptr);
      dqdpn0Ptr->linearCombo(sec.beta_[i],*(dqdpHistory[i]),1.0,*ds.qn0Ptr);
    }
  }
...
//-----------------------------------------------------------------------------
// Function      : N_TIA_Gear12::obtainSensitivityResiduals 
...
void N_TIA_Gear12::obtainSensitivityResiduals()
{
  ...
  for (int ip=0; ip<numParams;++ip)
  {
    ...
    N_LAS_Vector & lastDXdpVec = *(ds.lastDXdpPtrVector[ip]);

    std::vector<N_LAS_Vector*> & dqdpHistoryVec = ds.dqdpHistory[ip];

    N_LAS_Vector & currDQdxDXdpVec = *(ds.currDQdxDXdpPtrVector[ip]);
    N_LAS_Vector & lastDQdxDXdpVec = *(ds.lastDQdxDXdpPtrVector[ip]);
    ...
    N_LAS_Vector * tmpSolVectorPtr)
{
  ...
  {
    std::string msg = "N_TIA_Gear12::interpolateMPDESolution: ";
  ...
  *tmpSolVectorPtr = *(ds.xHistory[0]);
  ...
  for (int i=0; i < numblocks ; ++i)
  {
    ...
    for (int j=1 ; j <= kord ; ++j)
    {
      ...
      {
        Xyce::Report::DevelFatal0().in("N_TIA_Gear12::interpolateMPDESolution") << "N_LAS_Vector ds.xHistory[j] is not of type N_LAS_BlockVector\n j = " << j;
        return(false);
      }
...
/*
  ...
  if (tiaParams.debugLevel > 0)
  {
    ...
    Xyce::dout() << Xyce::section_divider << std::endl;
    Xyce::dout() <<
    "  N_TIA_Gear12::printMPDEOutputSolution" << std::endl;
  }
#endif // Xyce_DEBUG_TIME
  ...
  {
    std::string msg = "N_TIA_Gear12::printMPDEOutputSolution: ";
  ...
  for (unsigned int i=0; i < tinum ; ++i)
  {
    ...
    if ((s_ind == s_ind_0) || (i == tinum-1))
    {
      ...
      for (int j=0 ; j < num_interp_this_cycle ; ++j)
      {
        ...
        outputManagerAdapter->tranOutput(timept_, blockTmpSolVectorPtr->block(s), *ds.tmpStaVectorPtr, *ds.tmpStoVectorPtr  );
        total_interp++;
        s++;
        if (s >= blockCount) { s = 0; }
#ifdef Xyce_DEBUG_TIME
  ...
  if (fabs(timept_ - stop_time) >= eps)
  {
    ...
    outputManagerAdapter->tranOutput(timept_, blockTmpVecPtr->block(0), *ds.tmpStaVectorPtr, *ds.tmpStoVectorPtr  );
#ifdef Xyce_DEBUG_TIME
    Xyce::dout() << "Interpolated to t = " << timept_ << std::endl;
#endif // Xyce_DEBUG_TIME
  }
...
/*
  ...
  if (tiaParams.debugLevel > 0)
  {
    ...
    Xyce::dout() << Xyce::section_divider << std::endl;
    Xyce::dout() <<
    "  N_TIA_Gear12::printWaMPDEOutputSolution" << std::endl;
  }
#endif // Xyce_DEBUG_TIME
  double timestep = sec.lastAttemptedTimeStep;
  double lasttime = sec.currentTime - timestep;
  ...
  if (blockTmpSolVectorPtr == NULL)
  {
    std::string msg = "N_TIA_Gear12::printWaMPDEOutputSolution: ";
    msg += "N_LAS_Vector ds.tmpSolVectorPtr is not of type N_LAS_BlockVector";
    N_ERH_ErrorMgr::report(N_ERH_ErrorMgr::DEV_FATAL_0, msg);
    return(false);
  }
  ...
  for (int i=0 ; i < sec.usedOrder_ ; ++i)
  {
    ...
    for (unsigned int j=0 ; j < t1vals.size() ; ++j)
    {
      ...
      if (t >= 0.0) 
      {
        outputManagerAdapter->tranOutput(t, blockTmpSolVectorPtr->block(b1), *ds.tmpStaVectorPtr, *ds.tmpStoVectorPtr  );
#ifdef Xyce_DEBUG_TIME
...
//-----------------------------------------------------------------------------
// Function      : N_TIA_Gear12::updateSensitivityHistory
...
void N_TIA_Gear12::updateSensitivityHistory()
{
  ...
  for (int ip=0; ip<numParams;++ip)
  {
    std::vector<N_LAS_Vector*> & dqdpHistory = ds.dqdpHistory[ip];
    std::vector<N_LAS_Vector*> & dXdpHistory = ds.dXdpHistory[ip];
    std::vector<N_LAS_Vector*> & dQdxdXdpHistory = ds.dQdxdXdpHistory[ip];
...
void N_TIA_Gear12::initialize()
{
  ...
  if (tiaParams.errorAnalysisOption == 1)
  {
    Xyce::dout() << "ERROROPTION=1:  DeltaT Grow = 2" << "\n" << std::endl
      ...
      << "ERROROPTION=1:  NL MIN = " << tiaParams.NLmin << "\n" << std::endl
      << "ERROROPTION=1:  NL MAX = " << tiaParams.NLmax << "\n" << std::endl
...
//-----------------------------------------------------------------------------
// Function      : N_TIA_Gear12::initializeSensitivities
// Purpose       : 
// Special Notes :
// Scope         : public
// Creator       : Eric Keiter, SNL
...
void N_TIA_Gear12::initializeSensitivities()
{
  ...
  for (int ip=0; ip<numParams;++ip)
  {
    ...
    N_LAS_Vector * currDQdxDxdpPtr = ds.currDQdxDXdpPtrVector[ip];
...
void N_TIA_Gear12::rejectStep()
{
  ...
  if ((sec.stepAttemptStatus == false) && (adjustStep))
  {
    ...
    else
    {
      ...
      {
        std::string msg = "N_TIA_Gear12::rejectStep: ";
