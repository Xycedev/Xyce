...
// ---------------------
//
// Revision Number: $Revision: 1.112 $
//
// Revision Date  : $Date: 2014/05/21 22:10:14 $
//
...
// Creation Date : 9/5/02
//-----------------------------------------------------------------------------
double * N_LAS_Matrix::operator[]( int row ) const
{
  return (*oDCRSMatrix_)[row];
}
...
//-----------------------------------------------------------------------------
void N_LAS_Matrix::writeToFile(const char *filename, bool useLIDs, bool mmFormat )
{
  if (!mmFormat)
  {
