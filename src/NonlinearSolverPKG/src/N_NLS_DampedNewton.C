...
// ---------------------
//
// Revision Number: $Revision: 1.226 $
//
...
//-----------------------------------------------------------------------------
DampedNewton::DampedNewton(N_IO_CmdParse & cp):
  NonLinearSolver (cp),
  nlParams(DC_OP,cp),
  iNumCalls_(0),
  ...
  count (0)
{
  ...
  nlpMgrPtr_ = new ParamMgr (commandLine_);
...
//-----------------------------------------------------------------------------
bool DampedNewton::setOptions(const N_UTL_OptionBlock & OB)
{
...
//-----------------------------------------------------------------------------
bool DampedNewton::setTranOptions(const N_UTL_OptionBlock & OB)
{
...
//-----------------------------------------------------------------------------
bool DampedNewton::setHBOptions(const N_UTL_OptionBlock & OB)
{
...
//-----------------------------------------------------------------------------
// Function      : DampedNewton::spiceNewton_()
//
...
//-----------------------------------------------------------------------------
bool DampedNewton::spiceNewton_()
{
  // ***** Spice Newton method *****
  stepLength_ = 1.0;
...
//-----------------------------------------------------------------------------
// Function      : DampedNewton::bankRose_(nlInfo)
...
//-----------------------------------------------------------------------------
// Function      : DampedNewton::descent_(nlInfo)
// Purpose       :
// Special Notes :
// Scope         : private
// Creator       : Tamara G. Kolda, SNL, Compuational Sciences and
