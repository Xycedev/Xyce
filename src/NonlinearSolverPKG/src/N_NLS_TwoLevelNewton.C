...
// ---------------------
//
// Revision Number: $Revision: 1.135.2.1 $
//
// Revision Date  : $Date $
//
...
// Creation Date : 10/20/02
//-----------------------------------------------------------------------------
bool TwoLevelNewton::registerAnalysisManager
   (N_ANP_AnalysisManager * tiaPtr_tmp)
{
  bool bsuccess = true;
  bool tmpBool = true;

  tiaPtr_ = tiaPtr_tmp;
  tmpBool = nlsOuterPtr_->registerAnalysisManager(tiaPtr_);
  bsuccess = bsuccess && tmpBool;

  tmpBool = nlsInnerPtr_->registerAnalysisManager(tiaPtr_);
  bsuccess = bsuccess && tmpBool;
...
// Creation Date : 10/20/02
//-----------------------------------------------------------------------------
bool TwoLevelNewton::registerLinearSystem
   (N_LAS_System * ptr)
{
  bool bsuccess = true;
  bool tmpBool = true;
...
//-----------------------------------------------------------------------------
// Function      : TwoLevelNewton::registerLoader
// Purpose       :
// Special Notes :
// Scope         : public
// Creator       : Eric Keiter, SNL, Parallel Computational Sciences
// Creation Date : 10/20/02
//-----------------------------------------------------------------------------
bool TwoLevelNewton::registerLoader
...
//-----------------------------------------------------------------------------
bool TwoLevelNewton::registerOutputMgr (N_IO_OutputMgr * ptr)
{
  bool bsuccess = true;
  bool tmpBool = true;
...
// Creation Date : 10/20/02
//-----------------------------------------------------------------------------
bool TwoLevelNewton::setOptions(const N_UTL_OptionBlock & OB)
{
...
// Creation Date : 10/20/02
//-----------------------------------------------------------------------------
bool TwoLevelNewton::setTranOptions(const N_UTL_OptionBlock & OB)
{
  return nlsOuterPtr_->setTranOptions(OB);
}

bool TwoLevelNewton::setHBOptions(const N_UTL_OptionBlock & OB)
{
  return true;
}
...
//-----------------------------------------------------------------------------
bool TwoLevelNewton::setLocaOptions (const N_UTL_OptionBlock & OB)
{
...
// Creation Date : 09/10/03
//-----------------------------------------------------------------------------
bool TwoLevelNewton::setTwoLevelLocaOptions (const N_UTL_OptionBlock & OB)
{
...
// Creation Date : 10/20/02
//-----------------------------------------------------------------------------
bool TwoLevelNewton::setTwoLevelOptions(const N_UTL_OptionBlock & OB)
{
...
// Creation Date : 10/20/02
//-----------------------------------------------------------------------------
bool TwoLevelNewton::setTwoLevelTranOptions(const N_UTL_OptionBlock & OB)
{
...
//-----------------------------------------------------------------------------
// Function      : TwoLevelNewton::registerPetraOptions
...
bool TwoLevelNewton::setPetraOptions(const N_UTL_OptionBlock & OB)
{
  bool bsuccess = true;
  bool tmpBool = true;
  ...
  tmpBool = NonLinearSolver::setPetraOptions(OB);
  bsuccess = bsuccess && tmpBool;
...
//-----------------------------------------------------------------------------
int TwoLevelNewton::algorithm0_()
{
  int status = -1;
...
int TwoLevelNewton::algorithm4_()
{
       ...
       ++iter, ++iterFinalVal)
  {
	...
	if (successBool) // success!
	{
	  ...
	  outMgrPtr_->outputHomotopy(pdsMgrPtr_->getPDSComm()->comm(), paramNameList, paramCurrentVal, **nextSolVectorPtrPtr_);
	}
	else // failure!
	{
...
// Creation Date : 10/20/02
//-----------------------------------------------------------------------------
int TwoLevelNewton::solve (NonLinearSolver * nlsTmpPtr)
{
  int status = -1;
...
bool TwoLevelNewton::enableSensitivity ()
{
  ...
  Xyce::dout() << "TwoLevelNewton::enableSensitivity " << std::endl;
#endif
  bool bsuccess = true;
  bool tmpBool = true;
