...
//-----------------------------------------------------------------------------
// Function      : ConductanceExtractor::registerPkgOptionsMgr
// Purpose       :
// Special Notes :
// Scope         : public
...
//-----------------------------------------------------------------------------
bool ConductanceExtractor::registerPkgOptionsMgr( N_IO_PkgOptionsMgr *pkgOptPtr )
{
  ...
  pkgOptMgrPtr_->submitRegistration(
      "CONDUCTANCE", netListFile, new ConductanceExtractor_OptionsReg( this ) );
...
// Creation Date : 03/03/06
//-----------------------------------------------------------------------------
bool ConductanceExtractor::setOptions(const N_UTL_OptionBlock& OB)
{
  bool bsuccess = true;
...
// Creation Date : 03/03/06
//-----------------------------------------------------------------------------
bool ConductanceExtractor::setupIDs_
  (const std::map<std::string,double> & inputMap)
{
  ...
  for (; iterM != endM; ++i, ++iterM)
  {
    ...
    if (!(GIDList.empty ()))
    {
      ...
      if( vsrcPosLIDs_[i] == -1 )
      {
        std::string msg = "ConductanceExtractor::setupIDs_";
...

{
  ...
  if (debugLevel_ > 0)
  {
    ...
    cout << "ConductanceExtractor::extract" << std::endl;
    cout << subsection_divider << std::endl;
  }
#endif
...
//-----------------------------------------------------------------------------
// Function      : ConductanceExtractor::setupISO_IDs_
//
// Purpose       : This function sets up the various GIDs and matrix rows,
//                 that are needed in the extract function.
//
...
// Creation Date : 03/03/06
//-----------------------------------------------------------------------------
bool ConductanceExtractor::setupISO2_IDs_(const std::string & isoName)
{
  bool bsuccess = true;
  ...
  if (!(GIDList.empty ()))
  {
    ...
    for (i=0;i<idSize;++i,++gidExtIter)
    {
      ...
      if( vsrcPosLIDs_[i] == -1 )
      {
        std::string msg = "ConductanceExtractor::setupISO_IDs_";
        msg += " The " + sourceName + " source has the positive node"
         " owned by another processor.  The 2-level solve can't handle that.";
        N_ERH_ErrorMgr::report(N_ERH_ErrorMgr::DEV_FATAL, msg);
      }
...
( const std::string & isoName, std::vector< std::vector<double> > & jacobian )
{
  ...
  {
    cout << subsection_divider << std::endl;
    cout << "ConductanceExtractor::extract - iso2" << std::endl;
    cout << subsection_divider << std::endl;
  }
#endif
