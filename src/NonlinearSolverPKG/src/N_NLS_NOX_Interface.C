...
Interface::Interface(N_IO_CmdParse & cp) :
  ...
  dcParams_(Xyce::Nonlinear::DC_OP),
  transientParams_(Xyce::Nonlinear::TRANSIENT),
  hbParams_(Xyce::Nonlinear::HB_MODE),
  sharedSystemPtr_(0),
  mode_(Xyce::Nonlinear::DC_OP),
  lastParametersMode_(Xyce::Nonlinear::DC_OP),
  parametersMode_(Xyce::Nonlinear::DC_OP),
  usemode_(true),
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::spiceStrategy
//
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::stdNewtonSolve
// Purpose       : 
...
int Interface::stdNewtonSolve ( ParameterSet* paramsPtr )
{
  ...
  if (Teuchos::is_null(solverPtr_))
  {
    ...
    if ((usemode_) && (mode_ != Xyce::Nonlinear::TRANSIENT))
    {
      if (ICspecified_)
      {
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::naturalParameterContinuationSolve 
// Purpose       : 
// Special Notes : This corresponds to 
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::mosfetContinuationSolve 
// Purpose       : 
// Special Notes : This corresponds to 
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::mosfetContinuationSolve2 
// Purpose       : 
// Return Type   : 
// Special Notes : This corresponds to 
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::mosfetContinuationSolve3 
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::mosfetContinuationSolve4 
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::mosfetContinuationSolve5 
// Purpose       : 
// Return Type   : 
// Special Notes : This corresponds to 
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::mosfetContinuationSolve6 
// Purpose       : 
// Special Notes : This corresponds to 
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::blockGainscaleMosfetSolve 
// Purpose       : 
// Special Notes : This corresponds to 
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::gminSteppingSolve
// Purpose       : 
// Special Notes : This corresponds to 
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::pseudoTransientSolve
// Purpose       : 
// Special Notes : This corresponds to 
...
//-----------------------------------------------------------------------------
// Function      : N_NLS_NOX::Interface::artificialParameterHomotopy 
// Purpose       : 
// Special Notes : This corresponds to 
...
int Interface::solve (N_NLS_NonLinearSolver * nlsTmpPtr)
{
  ...
  try
  {
    ...
    if ((usemode_) && (mode_ == Xyce::Nonlinear::TRANSIENT))
    {
      ...
      parametersMode_ = Xyce::Nonlinear::TRANSIENT;
    }
    else if ((usemode_) && (mode_ == Xyce::Nonlinear::HB_MODE))
    {
      paramsPtr = &hbParams_;
      locaStatusTestPtr_ = locaHBStatusTestPtr_;
      lastParametersMode_ = parametersMode_;
      parametersMode_ = Xyce::Nonlinear::HB_MODE;
    }
    else
    {
      paramsPtr = &dcParams_;
    ...
    if (mode_ == Xyce::Nonlinear::DC_OP && !continuationSpecified)
    {
...
bool Interface::opStartCont0 (ParameterSet* paramsPtr)
{
  ...
  if (!DCOPused_)
  {
    ...
    const Xyce::NodeNamePairMap & allNodes = outMgrPtr_->getAllNodes( );
#ifdef Xyce_PARALLEL_MPI
    N_PDS_Comm * pdsCommPtr = pdsMgrPtr_->getPDSComm();
#endif

    if (found > 0 && icType == Xyce::IO::InitialConditionsData::IC_TYPE_DCOP_RESTART)
    {
      DCOPused_ = true;
      usedOP = true;
      Teuchos::RefCountPtr<N_NLS_NOX::AugmentLinSys> als =
...
bool Interface::icCont (ParameterSet* paramsPtr)
{
  ...
  usedIC = (icType==Xyce::IO::InitialConditionsData::IC_TYPE_IC && found > 0);
  if (usedIC)
  {
...
bool Interface::nodesetCont0 (ParameterSet* paramsPtr)
{
  ...
  usedNODESET = (icType==Xyce::IO::InitialConditionsData::IC_TYPE_NODESET && found > 0);
  if (usedNODESET)
  {
    Teuchos::RefCountPtr<N_NLS_NOX::AugmentLinSys> als =
...
// Creation Date :
//-----------------------------------------------------------------------------
void Interface::setAnalysisMode(Xyce::Nonlinear::AnalysisMode mode)
{
  mode_ = mode;
}
...
// Creation Date :
//-----------------------------------------------------------------------------
void Interface::resetAll (Xyce::Nonlinear::AnalysisMode mode)
{
...
{
  if (mode_ == Xyce::Nonlinear::DC_OP && setAZ_Tol_DC)
  {
    lasSolverPtr_->setTolerance(params.get("Tolerance", 1.0e-12));
  }
  else if (mode_ == Xyce::Nonlinear::TRANSIENT && setAZ_Tol_Transient)
  {
    lasSolverPtr_->setTolerance(params.get("Tolerance", 1.0e-12));
  }
...
bool Interface::getLocaFlag ()
{
  ...
  {
    if ((usemode_) && (mode_ == Xyce::Nonlinear::HB_MODE))
    {
      paramsPtr = &hbParams_;
    }
    else
