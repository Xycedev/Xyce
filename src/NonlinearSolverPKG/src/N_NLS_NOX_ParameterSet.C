...
// Creation Date :
//-----------------------------------------------------------------------------
ParameterSet::ParameterSet(Xyce::Nonlinear::AnalysisMode mode) :
 ...
 */
  ...
  */
  ...
  bool isTransient = false;
  if (mode_ == Xyce::Nonlinear::TRANSIENT)
  {
    isTransient = true;
  }
...
bool ParameterSet::setLocaOptions(const N_UTL_OptionBlock& OB, bool saveCopy)
{
       ...
       it_tpL != OB.getParams().end(); ++ it_tpL)
  {
    ...
    else if (tag == "RESIDUALCONDUCTANCE")
    {
      ...
      {
        Xyce::Report::UserWarning0() << "A non-zero value for the GMIN Stepping residual conductance has been specified (RESIDUALCONDUCTANCE= " << it_tpL->stringValue() << ")." << std::endl
...
ParameterSet::createAugmentLinearSystem(N_LAS_System* ls, Xyce::NodeNamePairMap & op,
                                        ...
                                        const Xyce::NodeNamePairMap & allNodes, N_PDS_Comm * pdsCommPtr
#else
