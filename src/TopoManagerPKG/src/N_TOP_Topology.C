...
//
// Revision Date  : $Date: 2014/07/15 19:03:38 $
//
...
//-----------------------------------------------------------------------------
// Function      : Topology::registerAnalysisManager
// Purpose       : Basic device processor addes dev node and connected v-nodes
// Special Notes :
// Scope         : public
// Creator       : Eric Keiter, SNL, Parallel Computational Sciences
// Creation Date : 6/8/00
//-----------------------------------------------------------------------------
bool Topology::registerAnalysisManager( N_ANP_AnalysisManager * anaInt)
{
...
void Topology::addDevice( const NodeBlock & nb, const Teuchos::RefCountPtr<Device::InstanceBlock> ibPtr )
{
  ...
  devInstMap_[ibPtr->getInstanceName().getEncodedName()] = 1;
			...
			const int * nodeGIDs, const int * procs )
{
  ...
  for( int i = 0; i < num; ++i )
  {
    ...
    else
    {
      ...
      migrateNodePtr->getDevBlock().setInstanceName(Device::InstanceName());
    }
