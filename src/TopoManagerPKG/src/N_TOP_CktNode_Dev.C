...
//
// Revision Date  : $Date: 2014/04/03 13:49:22 $
//
...
{
  // Get dependent solution variables from the device and the Depend structure to determine
  ...
  {
    bool found = false;
    std::vector< Depend > depParams = devPtr_->getDependentParams();
    std::vector< Depend >::iterator it_sdV = depParams.begin();
    std::vector< Depend >::iterator it_sdV_end = depParams.end();
