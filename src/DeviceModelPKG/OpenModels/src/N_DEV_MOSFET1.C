...
// ---------------------
//
// Revision Number: $Revision: 1.324 $
//
...
// Set up double precision variables:
    p.addPar ("TEMP", 0.0, false, ParameterType::TIME_DEP, &MOSFET1::Instance::temp, NULL,
       STANDARD, CAT_NONE, "Device temperature");
    p.addPar ("L",    0.0, true,    ParameterType::NO_DEP, &MOSFET1::Instance::l, NULL,
       U_METER,  CAT_GEOMETRY, "Channel length");
    p.addPar ("W",    0.0, true,    ParameterType::NO_DEP, &MOSFET1::Instance::w, NULL,
       U_METER,  CAT_GEOMETRY, "Channel width");
    p.addPar ("AD",   0.0, false,   ParameterType::NO_DEP, &MOSFET1::Instance::drainArea, NULL,
       U_METER2,  CAT_GEOMETRY, "Drain diffusion area");
    p.addPar ("AS",   0.0, false,   ParameterType::NO_DEP, &MOSFET1::Instance::sourceArea, NULL,
       U_METER2,  CAT_GEOMETRY, "Source diffusion area");
    p.addPar ("NRD",  1.0, false,   ParameterType::NO_DEP, &MOSFET1::Instance::drainSquares, NULL,
       U_SQUARES, CAT_GEOMETRY, "Multiplier for RSH to yield parasitic resistance of drain");
    p.addPar ("NRS",  1.0, false,   ParameterType::NO_DEP, &MOSFET1::Instance::sourceSquares, NULL,
       U_SQUARES, CAT_GEOMETRY, "Multiplier for RSH to yield parasitic resistance of source");
    p.addPar ("PD",   0.0, false,   ParameterType::NO_DEP, &MOSFET1::Instance::drainPerimeter, NULL,
       U_METER, CAT_GEOMETRY, "Drain diffusion perimeter");
    p.addPar ("PS",   0.0, false,   ParameterType::NO_DEP, &MOSFET1::Instance::sourcePerimeter, NULL,
       U_METER, CAT_GEOMETRY, "Source diffusion perimeter");
    p.addPar ("M",    1.0, false,   ParameterType::NO_DEP, &MOSFET1::Instance::numberParallel, NULL,
...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    ...
    if (getDeviceOptions().newMeyerFlag)
    {
      intNameMap[ li_Draindot ] = spiceInternalName(getName(), "draindot");
      intNameMap[ li_Gatedot ] = spiceInternalName(getName(), "gatedot");
      intNameMap[ li_Sourcedot ] = spiceInternalName(getName(), "sourcedot");
      intNameMap[ li_Bulkdot ] = spiceInternalName(getName(), "bulkdot");

      if ( li_DrainPrime != li_Drain )
      {
        intNameMap[ li_DrainPrimedot ] = spiceInternalName(getName(), "drainprimedot");
      }

      if ( li_SourcePrime != li_Source )
      {
        intNameMap[ li_SourcePrimedot ] = spiceInternalName(getName(), "sourceprimedot");
      }
    }
  }
...
bool Instance::updateIntermediateVars ()
{
...

  {
    ...
    if (getDeviceOptions().debugLevel > 0 && getSolverState().debugTimeFlag)
    {
      ...
      Xyce::dout() << "HOMOTOPY INFO: before vgs  = " << vgs << std::endl;
    }
#endif
