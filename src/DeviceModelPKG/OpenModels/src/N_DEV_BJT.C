...
// ---------------------
//
// Revision Number: $Revision: 1.388 $
//
...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty () && !externalNodeMode)
  {
    if( model_.collectorResist != 0.0 )
    {
      intNameMap[ li_CollP ] = spiceInternalName(getName(), "collectorprime");
    }

    if( model_.baseResist != 0.0 )
    {
      intNameMap[ li_BaseP ] = spiceInternalName(getName(), "baseprime");
    }

    if( model_.emitterResist != 0.0 )
    {
      intNameMap[ li_EmitP ] = spiceInternalName(getName(), "emitterprime");
    }
  }

  if (getDeviceOptions().newExcessPhase)
  {
    intNameMap [ li_Ifx ] = spiceInternalName(getName(), "ExcessPhase_Ifx");
    intNameMap [ li_dIfx ] = spiceInternalName(getName(), "ExcessPhase_dIfx");
  }
...
std::map<int,std::string> & Instance::getStoreNameMap ()
{
  ...
  {
    storeNameMap[ li_storevBE ] = spiceStoreName(getName(), "VBE");
    storeNameMap[ li_storevBC ] = spiceStoreName(getName(), "VBC");
    storeNameMap[ li_store_capeqCB ] = spiceStoreName(getName(), "CAPEQCB");

    if (loadLeadCurrent)
    {
      storeNameMap[ li_store_dev_ib ] = spiceStoreName(getName(), "DEV_IB");
      storeNameMap[ li_store_dev_ie ] = spiceStoreName(getName(), "DEV_IE");
      storeNameMap[ li_store_dev_ic ] = spiceStoreName(getName(), "DEV_IC");
