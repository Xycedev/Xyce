...
//
// Revision Date  : $Date: 2014/08/06 15:44:56 $
//
    ...
    breakTime(0.)
{
  ...
  {
    numInput = (dev_letter == 'Y') ? 2 : dev_numInputs;
    numOutput = 1;
    gate = AND;
  }
  ...
  if ((gate == AND) || (gate == NAND) || (gate == OR) || (gate == NOR))
  {
    ...
    if ( (dev_numInputs != 0) && (instance_block.numExtVars - iBase - numOutput != dev_numInputs) )
    {
      std::cout << instance_block.numExtVars << " " << dev_numInputs << " " << numExtVars << std::endl;
      UserError0(*this) << "too few I/O nodes on instance line.";
    }
  }
  // catch case where gates with a fixed number of inputs have
  // wrong number of inputs
  if (numExtVars != instance_block.numExtVars)
  {
    UserError0(*this) << "Incorrect number of nodes in digital device "
                      << ".  Found " << instance_block.numExtVars << ", should be " << numExtVars;
  }
...
bool Instance::updatePrimaryState ()
{
  ...
  for (int i=0 ; i<numInput ; ++i)
  {
    ...
    if (currentState == 0)
    {
      ...
      if ((-vcapref[i] > model_.s0vhi) && (-vcapref[i] > model_.s1vlo))
      {
          currentState = 1;
          changeState = true;
          if (DEBUG_DEVICE && getDeviceOptions().debugLevel > 1)
          {
  ...
  { // DLTCH: in0: PREB, in1: CLRB, in2: enable, in3: data
    ...
    else if (getSolverState().dcopFlag)
