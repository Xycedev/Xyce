...
// ---------------------
//
// Revision Number: $Revision: 1.235 $
//
// Revision Date  : $Date: 2014/08/10 14:05:49 $
//
  ...
  i_bra(0.0)
{
  ...
  {
    acSourceData_ = new ACData (IB.params,getSolverState(),getDeviceOptions());
  }

  if (DCSOURCETYPEgiven) // this will always be given, if the source spec was valid.
  {
    dcSourceData_ = new ConstData (IB.params,getSolverState(),getDeviceOptions());
  }
  ...
  if (getSolverState().HBspecified || TRANSIENTSOURCETYPEgiven)
  {
    ...
    {
      case _SIN_DATA:
        tranSourceData_ = new SinData (IB.params,getSolverState(),getDeviceOptions());
        break;

      case _EXP_DATA:
        tranSourceData_ = new ExpData (IB.params,getSolverState(),getDeviceOptions());
        break;

      case _PULSE_DATA:
        tranSourceData_ = new PulseData (IB.params,getSolverState(),getDeviceOptions());
        break;

      case _PWL_DATA:
        tranSourceData_ = new PWLinData (IB.params,getSolverState(),getDeviceOptions());
        break;

      case _SFFM_DATA:
        tranSourceData_ = new SFFMData (IB.params,getSolverState(),getDeviceOptions());
        break;

      case _DC_DATA:
        tranSourceData_ = new ConstData (IB.params,getSolverState(),getDeviceOptions());
        break;

      case _SMOOTH_PULSE_DATA:
        tranSourceData_ = new SmoothPulseData (IB.params,getSolverState(),getDeviceOptions());
        break;
...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    intNameMap[ li_Bra ] = spiceInternalName(getName(), "branch");
  }
...
bool Master::updateState (double * solVec, double * staVec, double * stoVec)
{
  for (InstanceVector::const_iterator it = getInstanceBegin(); it != getInstanceEnd(); ++it)
  {
    ...
    if ( (getSolverState().HBspecified || getSolverState().tranopFlag || getSolverState().transientFlag) && vi.tranSourceData_ != 0 )
    {
