...
// ---------------------
//
// Revision Number: $Revision: 1.245 $
//
...
void Traits::loadModelParameters(ParametricData<MOSFET3::Model> &p)
{
    ...
    p.addPar ("MJSW",   0.33,  false, ParameterType::NO_DEP,
    ...
    p.addPar ("ETA",    0.0,  false, ParameterType::NO_DEP,
    ...
    p.addPar ("KAPPA",    0.2,  false, ParameterType::NO_DEP,
...
//-----------------------------------------------------------------------------
std::map<int,std::string> & N_DEV_MOSFET3Instance::getStoreNameMap ()
{
  // set up the internal name map, if it hasn't been already.
...
bool Model::processParams ()
{
  ...
  if(oxideThickness == 0)
  {
    UserError0(*this) << getName() << " has TOX=0";
  }
  else
  {
