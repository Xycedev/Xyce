...
// ---------------------
//
// Revision Number: $Revision: 1.375 $
//
//
...
// Set up double precision variables:
    ...
    p.addPar ("NQSMOD", 0, false, NO_DEP,
...
void Traits::loadModelParameters(ParametricData<MOSFET_B3::Model> &p)
{
    ...
    p.addPar ("TOX", 150.e-10, true, NO_DEP,
      &MOSFET_B3::Model::tox,
      NULL,
    ...
    p.addPar ("XJ", 0.15e-6, false, NO_DEP,
      &MOSFET_B3::Model::xj,
    ...
    p.addPar ("KETA", -0.047, false, NO_DEP,
    ...
    p.addPar ("RDSW", 0.0, false, NO_DEP,
    ...
    p.addPar ("VOFFCV", 0.0, false, NO_DEP,
    ...
    p.addPar ("TCJ", 0.0, false, NO_DEP,
      &MOSFET_B3::Model::tcj,
      NULL, U_KM1, CAT_TEMP, "Temperature coefficient of Cj");

    p.addPar ("TCJSW", 0.0, false, NO_DEP,
    ...
    p.addPar ("TPB", 0.0, false, NO_DEP,
      &MOSFET_B3::Model::tpb,
      NULL, U_VKM1, CAT_TEMP, "Temperature coefficient of Pb");

    p.addPar ("TPBSW", 0.0, false, NO_DEP,
    ...
    p.addPar ("LALPHA1", 0.0, false, NO_DEP,
    ...
    p.addPar ("LVOFFCV", 0.0, false, NO_DEP,
    ...
    p.addPar ("WALPHA1", 0.0, false, NO_DEP,
    ...
    p.addPar ("WVOFFCV", 0.0, false, NO_DEP,
    ...
    p.addPar ("PALPHA1", 0.0, false, NO_DEP,
    ...
    p.addPar ("PVOFFCV", 0.0, false, NO_DEP,
...
#ifdef FRINGE_DONE
    p.addPar ("USEFRINGE", 0.0, false, NO_DEP,
    ...
    p.addPar ("CGBO", 0.0, false, NO_DEP,
    ...
    p.addPar ("CJSWG", 0.0, false, NO_DEP,
    ...
    p.addPar ("NOIC", 0.0, false, NO_DEP,
    ...
    p.addPar ("L", 5.e-6, false, NO_DEP,
      &MOSFET_B3::Model::model_l,
      NULL, U_METER,  CAT_GEOMETRY, "Channel length");

    p.addPar ("W", 5.e-6, false, NO_DEP,
    ...
    updateTemperatureCalled_ (false)
{
  ...
  if (given("NQSMOD"))
  {
    std::string msg = "Instance::Instance";
...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    ...
    if (icVDSGiven)
    {
      intNameMap[li_Ids] = spiceInternalName(getName(), "branch_DS");
    }

    if (icVGSGiven)
    {
      intNameMap[li_Igs] = spiceInternalName(getName(), "branch_GS");
    }

    if (icVBSGiven)
    {
      intNameMap[li_Ibs] = spiceInternalName(getName(), "branch_BS");
    }
  }
...
bool Instance::updateTemperature (const double & temp_tmp)
{
  ...
  else
  {
    ...
    {
      UserWarning0(*this) << "Effective channel length <= 0";
    }

    paramPtr->weff = w - 2.0 * paramPtr->dw;
    if (paramPtr->weff <= 0.0)
    {
      UserWarning0(*this) << "Effective channel width <= 0";
    }

    paramPtr->leffCV = l - 2.0 * paramPtr->dlc;
    if (paramPtr->leffCV <= 0.0)
    {
      UserWarning0(*this) << "Effective channel length for C-V <= 0";
    }
    ...
    {
      UserError0(*this) << "Error(s) detected during V3.2 parameter checking for " << name.c_str() << " in model " << model_.name;
#endif
      ...
      if (model_.k1Given || model_.k2Given)
      {
        if (!model_.k1Given)
        {
          UserWarning0(*this) << "k1 should be specified with k2.";
          paramPtr->k1 = 0.53;
        }

        if (!model_.k2Given)
        {
          UserWarning0(*this) << "k2 should be specified with k1.";
          paramPtr->k2 = -0.0186;
        }

        if (model_.nsubGiven)
        {
          UserWarning0(*this) << "nsub is ignored because k1 or k2 is given.";
        }

        if (model_.xtGiven)
        {
          UserWarning0(*this) << "xt is ignored because k1 or k2 is given.";
        }

        if (model_.vbxGiven)
        {
          UserWarning0(*this) << "vbx is ignored because k1 or k2 is given.";
        }

        if (model_.gamma1Given)
        {
          UserWarning0(*this) << "gamma1 is ignored because k1 or k2 is given.";
        }

        if (model_.gamma2Given)
        {
          UserWarning0(*this) << "gamma2 is ignored because k1 or k2 is given.";
        }
      }
      else
      {
  ...
  const Configuration & configuration,
    ...
    ni              (0.0)
    {
      ...
      if (!given("TNOM") && tnom == 0.0)
        Report::DevelFatal0() << "TNOM is zero";
