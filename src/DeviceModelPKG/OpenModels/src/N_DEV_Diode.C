...
// ---------------------
//
// Revision Number: $Revision: 1.304 $
//
...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    ...
    if ( li_Pos != li_Pri )
    {
      intNameMap[ li_Pri ] = spiceInternalName(getName(), "internal");
    }
  }
...
std::map<int, std::string> & N_DEV_DiodeInstance::getStoreNameMap ()
{
  ...
  {
    storeNameMap[ li_storevd ] = spiceStoreName(getName(), "vd");
    if( loadLeadCurrent )
    {
    ...
    )
{
  ...
  if(level == 1)
  {
    ...
    if (lambertWFlag == 1)
    {
      if (Vd >= -3.0 * Vte)
      {
        lambertWCurrent(Isat, Vte, tRS);
      }
      // linear reverse bias
      else if ( !tBrkdwnV || (Vd >= -tBrkdwnV) )
      {
        lambertWLinearReverseBias(Isat, Vte, tRS);
      }
      // reverse breakdown
      else
      {
        lambertWBreakdownCurrent(Isat, Vte, tRS);
      }
...
bool applyLimiters
( 
   N_DEV_DeviceSupport & devSupport,
   ...
   int & newtonIterOld, // instance variable
  ...
  )
{
  ...
  // update the "old" newton iteration number.
  if (newtonIter != 0 && newtonIter != newtonIterOld)
  {
    newtonIterOld = newtonIter;
  }
}
