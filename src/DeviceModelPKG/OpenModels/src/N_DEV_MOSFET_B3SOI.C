...
// ---------------------
//
// Revision Number: $Revision: 1.322 $
//
//
...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    if (bNode == 1)
    {
      intNameMap[li_Body] = spiceInternalName(getName(), "Body");
    }
    if (tNode == 1)
    {
      intNameMap[li_Temperature] = spiceInternalName(getName(), "Temp");
    }
    if (dNodePrime == 1)
    {
      intNameMap[li_DrainPrime] = spiceInternalName(getName(), "drain");
    }
    if (sNodePrime == 1)
    {
      intNameMap[li_SourcePrime] = spiceInternalName(getName(), "source");
    }
    if (gNodePrime == 1)
    {
      intNameMap[li_GatePrime] = spiceInternalName(getName(), "gate");
    }
    if (gNodeMid == 1)
    {
      intNameMap[li_GateMid] = spiceInternalName(getName(), "midgate");
    }
    if (icVDSGiven)
    {
    ...
    if (icVESGiven)
    {
      intNameMap[li_Ies] = spiceInternalName(getName(), "branch_ES");
    }
  // Spice model documentation says to ignore icVPS if
  // there isn't a p terminal.
    if (pNode != 0)
    {
      if (icVPSGiven)
      {
        intNameMap[li_Ips] = spiceInternalName(getName(), "branch_PS");
      }
    }
...
std::map<int,std::string> & Instance::getStoreNameMap ()
{
  ...
  if( loadLeadCurrent && storeNameMap.empty ())
  {
    ...
    storeNameMap[ li_store_dev_ie ] = spiceStoreName(getName(), "DEV_IE");
