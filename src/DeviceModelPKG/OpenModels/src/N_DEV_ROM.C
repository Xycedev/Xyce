...
//-------------------------------------------------------------------------
// Filename       : $RCSfile: N_DEV_ROM.C,v $
//
// Purpose        :
//
// Special Notes  :
...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    if (numIntVars > 0)  // Then add the current variables at the ports
    {
      for (int i=0; i<numExtVars; i++)
      {
        intNameMap[intLIDVec[i]] = spiceInternalName(getName(), "ip_Node"+Teuchos::Utils::toString(i+1));
      }
    }
    for (int i=0; i<numROMVars; i++)
    {
      intNameMap[li_ROM[i]] = spiceInternalName(getName(), "ROM_Node"+Teuchos::Utils::toString(i+1));
    }
  }
...
bool Instance::updatePrimaryState ()
{
  ...
  if (isGSparse)
    N_LAS_Util::crsAxpy( numROMVars, 1.0, &Ghat[0], &Ghat_rowPtr[0], &Ghat_colIdx[0], xhat, 0.0, &Fhat[numExtVars] );
  else
  ...
  if (isCSparse)
    N_LAS_Util::crsAxpy( numROMVars, 1.0, &Chat[0], &Chat_rowPtr[0], &Chat_colIdx[0], xhat, 0.0, &Qhat[0] );
  else
...
#pragma omp parallel for
  ...
  for (InstanceVector::const_iterator it = getInstanceBegin(); it != getInstanceEnd(); ++it)
  {
    ...
    if (ci.usePortDesc>0) // TWO-LEVEL DEVICE STAMPS
    {
      ...
      int sameTimeStep=0;
      if (ci.getSolverState().newtonIter==1) { ci.lastTimeStepNumber=ci.getSolverState().timeStepNumber; }
      if (ci.lastTimeStepNumber==ci.getSolverState().timeStepNumber) {sameTimeStep=1;}

      //********************************************************************
      // Set up the coefficients for discretization for the INNER solve
      //********************************************************************
      if (ci.usePortDesc==1)  {// DYNAMIC ORDER SELECTION
        if (sameTimeStep==0) {ci.coefLast = ci.coef;}
        ci.coef = (1/ci.getSolverState().currentOrder);
        if (ci.getSolverState().currentOrder==0) {  ci.coef=1; }     // Force BE
        if (ci.getSolverState().usedOrder==0) {	  ci.coefLast = 1; } // Force BE
      }
      ...
      ci.alph=1/ci.getSolverState().currTimeStep;
      if (ci.getSolverState().dcopFlag==1) { ci.alph=0.0; ci.alph_last=0.0; }
      if (ci.getSolverState().timeStepNumber==0) { ci.alph_last=0.0; }
      ...
      */
      ...
      if (updateCurrStaVec==1) {  // compute new `last' state
        ...
        int useOldA2=1;
        if (ci.getSolverState().timeStepNumber<2) { useOldA2=0; }
        if (ci.lastTimeStepNumber==ci.getSolverState().timeStepNumber) { useOldA2=0; }
      ...
      //**************************************************
      int updateA2 = 1;
      if (ci.getSolverState().timeStepNumber>2){
    ...
    if(ci.usePortDesc==0) // REGULAR DEVICE STAMPS
    {
      ...
      // Compute Fhat[numExtVars:numIntVars] = Ghat*xhat - Bhat*v_up
      if (ci.isGSparse)
        N_LAS_Util::crsAxpy( ci.numROMVars, 1.0, &ci.Ghat[0], &ci.Ghat_rowPtr[0], &ci.Ghat_colIdx[0], xhat, 0.0, &ci.Fhat[ci.numExtVars] );
      else
      ...
      // Compute Qhat = Chat * xhat
      if (ci.isCSparse)
        N_LAS_Util::crsAxpy( ci.numROMVars, 1.0, &ci.Chat[0], &ci.Chat_rowPtr[0], &ci.Chat_colIdx[0], xhat, 0.0, &ci.Qhat[0] );
      else
...
#pragma omp parallel for
        ...
        for (int j=0; j<ci.numExtVars; j++)
        {
          ...
          *(ci.fROMEqu_Bhat_VarsPtrs[2*i+j]) -= ci.Bhat[j*ci.numROMVars + i];
        }
      }
