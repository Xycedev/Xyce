...
//-------------------------------------------------------------------------
// Filename       : $RCSfile: N_DEV_VDMOS.C,v $
//
...
// ---------------------
//
// Revision Number: $Revision: 1.160 $
//
...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    ...
    {
      intNameMap[ li_GatePrime ] = spiceInternalName(getName(), "gateprime");
    }
    ...
    {
      intNameMap[ li_DrainDrift ] = spiceInternalName(getName(), "draindrift");
    }

    if ( li_D1Prime != li_Source )
    {
      intNameMap[ li_D1Prime ] = spiceInternalName(getName(), "d1pos");
    }
  }
...
void Instance::registerStoreLIDs( const std::vector<int> & stoLIDVecRef )
{
  ...
  if( loadLeadCurrent )
  {
    li_store_dev_id = stoLIDVec[0];
    li_store_dev_ig = stoLIDVec[1];
    li_store_dev_is = stoLIDVec[2];
    li_store_dev_ib = stoLIDVec[3];
  }
}
...
bool Instance::loadDAEQVector ()
{
  ...
  if( loadLeadCurrent )
  {
    ...
    {
      // D1' is S
      storeLeadQ[li_store_dev_is] += D1DIOcapCharge;
    }
  }
...
bool Instance::loadDAEFVector ()
{
  ...
  if( loadLeadCurrent )
  {
    ...
    {
      // D1' is S
      storeLeadF[li_store_dev_is] += (D1current - Ird1rs);
    }
  }
...
#define ISUBMOD  model_.isubmod
  ...
  if (getDeviceOptions().voltageLimiterFlag)
  {
    ...
    if (tmpGiven && (D1vd < Xycemin(0.0, 10.0*D1vte-D1DIOtBrkdwnV) ))
    {
  ...
  if(model_.artd != 0.0 && sourceArea != 0.0)
  {
    ...
    end1=1+exp(Xycemin(CONSTMAX_EXP_ARG,end10/vt));
    end20=model_.brtd-model_.crtd-model_.nrtd*vbs;
    end2=1+exp(Xycemin(CONSTMAX_EXP_ARG,end20/vt));
...
//    cbs = 0.0;
  ...
  {
    evbs = exp(Xycemin(CONSTMAX_EXP_ARG,model_.n2*vbs/vt));
...
//    cbd = 0.0;
  ...
  {
    evbdd = exp(Xycemin(CONSTMAX_EXP_ARG,model_.n2*vbdd/vt));
...
//#define EPSSIO2    3.453130e-11 == CONSTEPSOX (Xyce value)
  ...
  if(gammal != 0.0) {
    ...
    vthLimit = N_UTL_MachineDependentParams::DoubleMax();
...
// diode #1 quatntities
  ...
  if( !getSolverState().dcopFlag && getSolverState().initTranFlag && !getSolverState().newtonIter )
  {
...
bool Instance::updateSecondaryState ()
{
  ...
  double Dtype =model_.dtype;
  double D1current = Dtype*(D1cdeq + D1DIOcapCurrent);

  // Even though these do not look exactly like the RHS loads, they do account
  // for the currents into the leads even with the lead resistors in place
  ...
  LeadCurrentB = model_.dtype*(cbd-cbs-cqgb+cqbd+cqbs);
  */
  ...
  for (InstanceVector::const_iterator it = getInstanceBegin(); it != getInstanceEnd(); ++it)
    {
      ...
      double D1current = Dtype*(mi.D1cdeq + mi.D1DIOcapCurrent);
      ...
      /*
      Lead current load has been moved to F & Q loads
      mi.LeadCurrentD = -mi.getModel().dtype*(-mi.mode*mi.cdrain+mi.cbd+mi.cqgdd+mi.cqbd)-D1current-mi.Irdsshunt;
      mi.LeadCurrentG = mi.getModel().dtype*(mi.cqgs+mi.cqgb+mi.cqgdd);
      mi.LeadCurrentS = mi.getModel().dtype*(-mi.mode*mi.cdrain+mi.cbs-mi.cqgs-mi.cqbs) + D1current+mi.Irdsshunt;
...
bool Master::loadDAEVectors (double * solVec, double * fVec, double *qVec,  double * bVec, double * storeLeadF, double * storeLeadQ)
{
  for (InstanceVector::const_iterator it = getInstanceBegin(); it != getInstanceEnd(); ++it)
  {
    ...
    if( mi.loadLeadCurrent )
    {
      ...
      {
        // D1' is S
        storeLeadQ[mi.li_store_dev_is] += mi.D1DIOcapCharge;
      }
    }
