...
//
// Revision Date  : $Date: 2014/05/13 14:50:41 $
//
...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    for (int i=0; i<numLumps; i++)
    {
      ...
      {
        if (i != 0)
          intNameMap[lumpVec[i].li_V1] = spiceInternalName(getName(), "v1_lump" + Teuchos::Utils::toString(i));
        intNameMap[lumpVec[i].li_V2] = spiceInternalName(getName(), "v2_lump" + Teuchos::Utils::toString(i));
        intNameMap[lumpVec[i].li_I ] = spiceInternalName(getName(), "I__lump" + Teuchos::Utils::toString(i));
      }
      else if ( model_.specialCase == TRANS_MOD_LC)
      {
        if (i != 0)
