...
//-------------------------------------------------------------------------
// Filename       : $RCSfile: N_DEV_ArtificialParameters.C,v $
//
...
//
// Revision Date  : $Date: 2014/04/28 21:48:24 $
//
...
{
  getSolverState(device_manager).gainScale[0] = value;
  getSolverState(device_manager).artParameterFlag = true;
  return true;
}

double MOSFETGainScaleParam::getValue(const DeviceMgr &device_manager) const
{
  return getSolverState(device_manager).gainScale[0];
}

bool MOSFETNLTermScaleParam::setValue(DeviceMgr &device_manager, double value)
{
  getSolverState(device_manager).nltermScale = value;
  getSolverState(device_manager).artParameterFlag = true;
  return true;
}
...
{
  return getSolverState(device_manager).nltermScale;
}
...
bool MOSFETLParam::setValue(DeviceMgr &device_manager, double value)
{
  ...
  getSolverState(device_manager).sizeParameterFlag = true;
  ...
  if (model_group_it != getModelTypeInstanceVectorMap(device_manager).end()) {
    for (DeviceMgr::InstanceVector::const_iterator it = (*model_group_it).second.begin(); it != (*model_group_it).second.end(); ++it)
    {
...
bool MOSFETSizeScaleParam::setValue(DeviceMgr &device_manager, double value)
{
  ...
  getSolverState(device_manager).sizeParameterFlag = true;
  getSolverState(device_manager).sizeScale = value;
...
bool BJTExpOrdParam::setValue(DeviceMgr &device_manager, double value)
{
  getDeviceOptions(device_manager).exp_order = value;
  getSolverState(device_manager).bjtArtParameterFlag = true;
  return true;
}

double BJTExpOrdParam::getValue(const DeviceMgr &device_manager) const
{
  return 0.0;
}
...
{
  getSolverState(device_manager).pdeAlpha = value; // not important - part of planned refactor.

  if (!getSolverState(device_manager).PDEcontinuationFlag)
  {
...
{
  return getSolverState(device_manager).pdeAlpha;
}

bool PDEBetaParam::setValue(DeviceMgr &device_manager, double value)
{
  getSolverState(device_manager).PDEcontinuationFlag = true;
...
{
  getSolverState(device_manager).chargeAlpha = value;
  getSolverState(device_manager).chargeHomotopy = true;
...
{
  return getSolverState(device_manager).chargeAlpha;
}
