...
//-------------------------------------------------------------------------
// Filename       : $RCSfile: N_DEV_JFET.C,v $
//
// Purpose        :
//
// Special Notes  :
...
void Traits::loadInstanceParameters(ParametricData<JFET::Instance> &p)
{
  ...
  p.addPar("AREA", 1.0, &JFET::Instance::area)
    .setUnit(U_METER2)
...
std::ostream &Model::printOutInstances(std::ostream &os) const
{
  ...
  os << std::endl;
  os << "Number of JFET Instances: " << isize << std::endl;
    ...
    li_state_gcgd(-1)
{
  ...
  numStateVars = 4;
  setNumStoreVars(2);
  numLeadCurrentStoreVars = 3; // drain, source, gate lead currents
...
//-----------------------------------------------------------------------------
// Function      : N_DEV_JFETInstance::getStoreNameMap
// Purpose       :
// Special Notes :
// Scope         : public
...
//-----------------------------------------------------------------------------
std::map<int,std::string> & N_DEV_JFETInstance::getStoreNameMap ()
{
  // set up the internal name map, if it hasn't been already.
...
// for the Shockley version
  ...
  if (getDeviceOptions().debugLevel > 0 && getSolverState().debugTimeFlag)
  {
    ...
    Xyce::dout() <<"  Instance::updateIntermediateVars.  name = " << getName() << std::endl;
  ...
  if (getDeviceOptions().debugLevel > 0 && getSolverState().debugTimeFlag)
  {
    if (origFlag == 0)
    {
      Xyce::dout() << getName() << " Something modified the voltages. " << std::endl;
    }
    else
    {
      Xyce::dout() << getName() << " No voltages were limited. " << std::endl;
    }
    Xyce::dout() << " Voltage       before                    after                   diff " << std::endl;
    Xyce::dout() << getName() <<" vgs  " <<vgs_orig <<"  " << vgs <<" " << vgs-vgs_orig << std::endl;
    Xyce::dout() << getName() <<" vgd  " <<vgd_orig <<"  " << vgd <<" " << vgd-vgd_orig << std::endl;
    Xyce::dout() << getName() <<" vds  " <<vds_orig <<"  " << vds <<" " << vds-vds_orig << std::endl;
    Xyce::dout() << " " << std::endl;
  }
#endif
  ...
  level  =  model_.getLevel();
  if(level == 1)
  {
    //
    // Sydney University JFET model
    //
    if(vds >= 0)    // normal mode
    {
      ...
      if (vgst <= 0)
      {
        ...
        gm=0;
        gds=0;
        // Xyce::dout() << " cutoff region - normal mode " << std::endl;
      }
      else
      {
        betap=tBeta*(1+tLambda*vds);
        Bfac = (1.0-tJFETb)/(tPB-tvt0);
        if( vgst >= vds )
        {
          //
          //     linear region
          //
          apart=2.0*tJFETb+3.0*Bfac*(vgst-vds);
          cpart=vds*(vds*(Bfac*vds - tJFETb)+vgst*apart);
          cdrain=betap*cpart;
          gm=betap*vds*(apart+3.0*Bfac*vgst);;
          gds=betap*(vgst-vds)*apart+tLambda*tBeta*cpart;
         //  Xyce::dout() << " linear region - normal mode" << std::endl;
        }
        else
        {
          //
          ...
          //
          Bfac=vgst*Bfac;
          gm=betap*vgst*(2.0*tJFETb+3.0*Bfac);
          cpart=vgst*vgst*(tJFETb+Bfac);
          cdrain=betap*cpart;
          gds=tLambda*tBeta*cpart;
          // Xyce::dout() << " saturation region - normal mode " << std::endl;
        }
      }
    }
    else   // inverse mode
//  Inverse mode is where you switch the drain and source
//  and run the device in reverse.  The equations for inverse
//  mode are obtained by the following substitutions:
//  vgst -> vgdt, vds -> -vds, cdrain -> -cdrain
    {
      vgdt=vgd-tvt0;
      if (vgdt <= 0)
      {
        ...
        gm=0;
        gds=0;
        // Xyce::dout() << " cutoff region - inverse mode" << std::endl;
      }
      else
      {
        betap=tBeta*(1-tLambda*vds);
        ...
        if (vgdt+vds >= 0)
        {
          ...
          //     linear region
          //
          apart=2.0*tJFETb+3.0*Bfac*(vgdt+vds);
          cpart=vds*(-vds*(-Bfac*vds - tJFETb)+vgdt*apart);
          cdrain=betap*cpart;
          gm=betap*vds*(apart+3.0*Bfac*vgdt);;
          gds=betap*(vgdt+vds)*apart-tLambda*tBeta*cpart - gm;
          // Xyce::dout() << " linear region - inverse mode" << std::endl;
        }
        else
        {
          //
          ...
          //
          Bfac=vgdt*Bfac;
          gm=-betap*vgdt*(2.0*tJFETb+3.0*Bfac);
          cpart=vgdt*vgdt*(tJFETb+Bfac);
          cdrain=-betap*cpart;
          gds=tLambda*tBeta*cpart - gm;
          // Xyce::dout() << " saturation region - inverse mode " << std::endl;
        }
      }
    }
  }
  ...
  {
    //
    // Shockley JFET model with modified Vdsat
    //
    if(vds >= 0)    // normal mode
    {
      vgst=vgs-tvt0;
      if (vgst <= 0)
      {
        ...
        gm=0;
        gds=0;
         // Xyce::dout() << " cutoff region - Shockley normal mode " << std::endl;
      }
      else
      {
        A = tPB-tvt0;
        ...
        betap=tBeta*(1+tLambda*vds)/(1+tTheta*vgst);
        Vdsat = vgst + tDelta;
   //     Idsat = dtype*betap*(vgst-(2*A/3)*(1-B*B12));

        if(A != 0) C = (vds - tDelta*vds/Vdsat + A-vgst)/A;
        else C = 0;
        if(C >= 0) C12 = sqrt(C);
        else C12 = 0;
        D = C*C12 - B*B12;
        ...
        if( dtype*Vd <= Vdsat )
        {
          ...
          //     linear region
          //
          cdrain = betap*(vds - tDelta*vds/Vdsat - (2*A/3)*D);
          gm  = -cdrain*tTheta/(1+tTheta*vgst) + betap*(C12 - B12)
               + betap*tDelta*(1-C12)*vds/(Vdsat*Vdsat);
          gds = tLambda*cdrain/(1+tLambda*vds)
               + betap*(1-(1-C12)*tDelta/Vdsat-C12);
          // Xyce::dout() << " linear region - Shockley normal mode" << std::endl;
        }
        else
        {
          //
          //     saturation region
          //
          cdrain = betap*(vgst - (2*A/3)*(1 - B*B12));
          gm  = betap*(1-B12) - tTheta*cdrain/(1+tTheta*vgst);
          gds = tLambda*cdrain/(1+tLambda*vds);
          // Xyce::dout() << " saturation region - Shockley normal mode" << std::endl;
        }
      }
    }
    else   // inverse mode
...
//  vgst -> vgdt, vds -> -vds, cdrain -> -cdrain
    {
      ...
      if (vgdt <= 0)
      {
        ...
        gm=0;
        gds=0;
        // Xyce::dout() << " cutoff region - Shockley inverse mode " << std::endl;
      }
      else
      {
        A = tPB-tvt0;
        ...
        betap = tBeta*(1-tLambda*vds)/(1+tTheta*vgdt);
        Vdsat = vgdt + tDelta;
    //    Idsat = -dtype*betap*(vgdt-(2*A/3)*(1-B*B12));

        if(A != 0) C = (-vds + tDelta*vds/Vdsat + A-vgdt)/A;
        else C = 0;
        if(C >= 0) C12 = sqrt(C);
        else C12 = 0;
        D = C*C12 - B*B12;
        ...
        if( dtype*Vs <= Vdsat )
        {
          ...
          //
          cdrain = -betap*(-vds + tDelta*vds/Vdsat - (2*A/3)*D);
          gm = -betap*(C12 - B12) - tTheta*cdrain/(1+tTheta*vgdt);
          gm += betap*(1-C12)*tDelta*vds/(Vdsat*Vdsat);
          gds = betap*(1 - (1-C12)*tDelta/Vdsat - C12)
                                   - tLambda*cdrain/(1-tLambda*vds);

          // Xyce::dout() << " linear region - Shockley inverse mode" << std::endl;
        }
        else
        {
          //
          //     saturation region
          //
          cdrain = -betap*(vgdt - (2*A/3)*(1 - B*B12));
          gm = -betap*(1-B12) - tTheta*cdrain/(1+tTheta*vgdt);
          gds = tLambda*cdrain/(1-tLambda*vds);
          // Xyce::dout() << " saturation region - Shockley inverse mode" << std::endl;
        }
      }
    }
  }
  ...
  if (getDeviceOptions().debugLevel > 0 && getSolverState().debugTimeFlag)
  {
    Xyce::dout() << " Done with Instance::updateIntermediateVars.  name = " << getName() << std::endl;
...
bool Instance::loadDAEQVector ()
{
  ...
  if( loadLeadCurrent )
  {
    double * stoVec = extData.storeLeadCurrQCompRawPtr;
    stoVec[li_store_dev_id] = -(-cdreq+ceqgd);
    stoVec[li_store_dev_is] = -(cdreq+ceqgs);
    stoVec[li_store_dev_ig] = ceqgs+ceqgd;
  }
 ...
 if( loadLeadCurrent )
  {
    ...
    if (drainCond  != 0.0)
    {
      stoVec[li_store_dev_id] = Idrain;
    }
    else
    {
      stoVec[li_store_dev_id] = -(Idrain +(-cdreq+ceqgd));
    }
    if (sourceCond != 0.0)
    {
      stoVec[li_store_dev_is] = Isource;
    }
    else
    {
      stoVec[li_store_dev_is] = -(Isource+(cdreq+ceqgs));
    }
    stoVec[li_store_dev_ig] = (ceqgs+ceqgd);
  }
...
//    Xyce::dout() << subsection_divider << std::endl;
  ...
  tDelta  =  model_.DELTA;       // parameter to shift Shockley Vdsat
  tTheta  =  model_.THETA;       // gate voltage mobility parameter
  tRD     =  model_.RD/area;     // drain ohmic resistance
  tRS     =  model_.RS/area;     // source ohmic resistance
  tJFETb  =  model_.B;           // dopinng tail parameter
  ...
  if (getDeviceOptions().debugLevel > 0 && getSolverState().debugTimeFlag)
  {
    ...
    Xyce::dout() << "tJFETb  = " << tJFETb << std::endl;
    Xyce::dout() << "tLambda = " << tLambda << std::endl;
    Xyce::dout() << "tDelta  = " << tDelta << std::endl;
...
void registerDevice()
{
  Config<Traits>::addConfiguration()
    .registerDevice("j", 1)
    .registerDevice("j", 2)
    .registerModelType("pjf", 1)
    .registerModelType("njf", 1)
    .registerModelType("pjf", 2)
    .registerModelType("njf", 2);
}
