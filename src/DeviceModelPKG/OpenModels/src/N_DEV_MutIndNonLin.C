...
//
// Revision Date  : $Date: 2014/08/07 17:07:36 $
//
...
void Traits::loadModelParameters(ParametricData<MutIndNonLin::Model> &p)
{
    ...
    p.addPar ("CLIM", 0.005, false, ParameterType::NO_DEP,
      &MutIndNonLin::Model::CLim,
      NULL, U_NONE, CAT_MATERIAL, "Value below which domain flexing parameter will be treated as zero.");
    ...
    p.addPar ("CONSTDELVSCALING", false, false, ParameterType::NO_DEP,
    ...
    p.addPar ("INCLUDEMEQU", true, false, ParameterType::NO_DEP,
      &MutIndNonLin::Model::includeMEquation,
      &MutIndNonLin::Model::includeMEquationGiven, 
      U_NONE, CAT_NONE, "Flag to include the magnetics in the solution.");
...
std::map<int,std::string> & Instance::getIntNameMap()
{
  ...
  if (intNameMap.empty ())
  {
    ...
    {
      intNameMap[ (*it)->li_Branch ] = spiceInternalName(getName(), (*it)->name + "_branch");
    }
    if( model_.includeMEquation )
    {
      intNameMap[ li_MagVar ] = spiceInternalName(getName(), "m");
    }
...
std::map<int,std::string> & Instance::getStateNameMap()
{
  ...
  {
    stateNameMap[ li_MagVarState ] = spiceInternalName(getName(), "m");
    stateNameMap[ li_MagVarDerivState ] = spiceInternalName(getName(), "dmdt");
  }
...
std::map<int,std::string> & Instance::getStoreNameMap()
{
  ...
  {
    storeNameMap[ li_RVarStore ] = getName().getEncodedName() + "_r";
    storeNameMap[ li_BVarStore ] = getName().getEncodedName() + "_b";
    storeNameMap[ li_HVarStore ] = getName().getEncodedName() + "_h";
  }
  return storeNameMap;
}
