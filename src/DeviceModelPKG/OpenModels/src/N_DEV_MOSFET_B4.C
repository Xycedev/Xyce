...
// ---------------------
//
// Revision Number: $Revision: 1.103 $
//
//
...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    ...
    if (rbodyMod)
    {
      ...
      intNameMap[ li_SourceBody ] = spiceInternalName(getName(), "SourceBody");
      intNameMap[ li_DrainPrime ] = spiceInternalName(getName(), "DrainBody");
    }

    if (trnqsMod)
    {
      intNameMap[li_Charge] = spiceInternalName(getName(), "charge");
    }
