...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    ...
    for( int i=0; i<nSeg; i++)
    {
      ...
      segNumber << i;
      intNameMap[ li_Vol[i] ] = spiceInternalName(getName(), "V" + segNumber.str());
      intNameMap[ li_nPro[i] ] = spiceInternalName(getName(), "N" + segNumber.str());
      intNameMap[ li_mPro[i] ] = spiceInternalName(getName(), "M" + segNumber.str());
      intNameMap[ li_hPro[i] ] = spiceInternalName(getName(), "H" + segNumber.str());
    }
  }
  return intNameMap;
}
