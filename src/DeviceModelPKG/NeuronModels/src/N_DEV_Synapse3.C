...
//-------------------------------------------------------------------------
// Filename       : $RCSfile: N_DEV_Synapse3.C,v $
//
    ...
    randInitialized(false)
{
  ...
  // TODO - should 'factor' initialization be here instead of in processParams?
  processParams ();
}
...
// copy over the global ID lists.
  ...
  if( loadLeadCurrent )
  {
    li_store_dev_i = stoLIDVec[7];
  }
}


//-----------------------------------------------------------------------------
// Function      : MutIndNonLinInstance::getStateNameMap
// Purpose       :
// Special Notes :
// Scope         : public
// Creator       : Rich Schiek, SNL, Electrical Systems Modeling
...
std::map<int,std::string> & Instance::getStoreNameMap()
{
  ...
  if (storeNameMap.empty ())
  {
    ...
    storeNameMap[ li_weight_store ] = getName().getEncodedName() + "_W";
    storeNameMap[ li_VL1_store ] = getName().getEncodedName() + "_VL1";
    storeNameMap[ li_VL2_store ] = getName().getEncodedName() + "_VL2";
    storeNameMap[ li_VL3_store ] = getName().getEncodedName() + "_VL3";
    if( loadLeadCurrent )
    {
  ...
  else
  {
    ...
    if (ready)
    {
      if (vPre > vThresh)
      {
        ...
        {
          double arand = devSupport.getRandomPerturbation(); // get a number from [0, 1)
    ...
    else  // already had spike start, looking for end
    {
      ...
      if( (vPre < vThresh) && ((time - lastStoVec[li_t0_store]) > ( model_.tau1 + model_.tau2 )))
      {
        ready=true;
      }
    }
    ...
    vl1Update = getSolverState().currTimeStep * (vPost - vl1Now) / model_.vL1tau1;
    vl2Update = getSolverState().currTimeStep * (vPost - vl2Now) / model_.vL2tau2;
    if( vPre > model_.sParam )
    {
      vl3Update = getSolverState().currTimeStep * (1.0 - vl3Now) / model_.vL3tau3;
    }
    else
    {
      vl3Update = getSolverState().currTimeStep * (-vl3Now) / model_.vL3tau3;
    }
...
bool Instance::outputPlotFiles ()
{
  ...
  if (time >= respondTime)
  {
    ...
    stoVec[li_t0_store] = getSolverState().currTime;
