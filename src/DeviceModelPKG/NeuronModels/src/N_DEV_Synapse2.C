...
//----------------------------------------------------------------------------
// Filename       : $RCSfile: N_DEV_Synapse2.C,v $
//
// Purpose        :
//
// Special Notes  :
...
void Traits::loadInstanceParameters(ParametricData<Synapse2::Instance> &p)
{
  // Set up configuration constants:
}

void Traits::loadModelParameters(ParametricData<Synapse2::Model> &p)
{
  // Set up map for double precision variables:
  // Default values are taken from Destexhe 98, values for AMPA receptor
  // although since this Synapse2::Model is different, those values may not have the same meaning
  p.addPar ("GMAX", 1.0E-9, &Synapse2::Model::gMax)
    .setUnit(U_OHMM1)
    .setDescription("Maximal Synaptic Conductance");

  p.addPar ("EREV", 0.0, &Synapse2::Model::eRev)
    .setUnit(U_VOLT)
    .setDescription("Reversal Potential");

  p.addPar ("ALPHA", 1.1E-6, &Synapse2::Model::alpha)
    .setUnit(U_SECM1)
    .setDescription("Forward rate constant for receptor opening");

  p.addPar ("BETA", 190.0, &Synapse2::Model::beta)
    .setUnit(U_SECM1)
    .setDescription("Backward rate constant for receptor opening");

  p.addPar ("VP", 0.002, &Synapse2::Model::vP)
    .setUnit(U_VOLT)
    .setDescription("Presynaptic voltage that activates postsynaptic current");

  p.addPar ("TMAX", 0.001, &Synapse2::Model::tMax)
...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    intNameMap[ li_rVar ] = spiceInternalName(getName(), "r");
  }
  return intNameMap;
}
...
// Purpose       : Note that the Synapse2 does not have any state vars.
// Special Notes :
// Scope         : public
// Creator       : Robert Hoekstra, SNL, Parallel Computational Sciences
...
bool Instance::updateIntermediateVars()
{
  ...
  {
    // no firing during DCOP; r=0, so postsynaptic current is 0 and unchanging
    ipost = 0.0;
    didVpost = 0.0;
    didr = 0.0;
    rFval = 0.0;
  ...
  else
  {
    ...
    }
    {
      ...
      Sacado::Fad::SFad<double,2> resultFad;
      resultFad = rEquF( vPreVar, rVarS, alphaVar, betaVar, tMaxVar, vPVar);
      rFval = resultFad.val();
...
//                 Synapse2  instance.
//
// Special Notes : This is an algebraic constaint, and as such the Synapse2
...
//                 distinct charges.
//
//                 This function assumes that updatePrimaryState has been
//                 called recently enough for the contents of q0 to be
//                 valid.
//
// Scope         : public
// Creator       : Eric Keiter, SNL, Parallel Computational Sciences
...
std::ostream &Model::printOutInstances(std::ostream &os) const
{
  ...
  os << std::endl;
  os << "Number of Synapse2 Instances: " << isize << std::endl;
...
void registerDevice()
{
  Config<Traits>::addConfiguration()
    .registerDevice("synapse", 2)
    .registerModelType("synapse", 2);
}
