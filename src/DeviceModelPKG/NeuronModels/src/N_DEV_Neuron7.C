...
void Traits::loadInstanceParameters(ParametricData<Neuron7::Instance> &p)
{
...
void Traits::loadModelParameters(ParametricData<Neuron7::Model> &p)
{
  p.addPar ("MEMC", 0.0, false, ParameterType::NO_DEP,
...
std::map<int,std::string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    intNameMap[ li_U ] = spiceInternalName(getName(), "U");
  }
...
void registerDevice()
{
  Config<Traits>::addConfiguration()
    .registerDevice("neuron", 7)
    .registerModelType("neuron", 7);
}
