...
inline int ReactionNetwork::getReactantNum(const std::string &name)
{
  ...
  if (i == -1)
  {
    ...
    if (i == -1)
    {
      std::string msg="ReactionNetwork::getReactantNum: invalid species name specified: ";
      msg += name;
...
// Creation Date : 3/20/06
//-----------------------------------------------------------------------------
inline N_DEV_Reaction & ReactionNetwork::getReaction(int i)
{
...
// Creation Date : 3/20/06
//-----------------------------------------------------------------------------
inline  N_DEV_Reaction &ReactionNetwork::getReaction(const std::string &name)
{
  int ni;
  ...
  if (ni == -1)
  {
    ...
    ost << " Attempt to access non-existant reaction " << name << std::endl;
...
{
  //getReaction(name).setCaptureRateCalculator(sigma,20335471.413078606,C0,t0,x0);  //LCM
...
{
  //getReaction(name).setCaptureRateCalculator(sigma,16805108.930336751,C0,t0,x0);  //LCM
...
{
  //getReaction(name).setEmissionRateCalculator(sigma,20335471.413078606,2.86e19,E,C0,t0,x0);  //LCM
...
{
  //getReaction(name).setEmissionRateCalculator(sigma,16805108.930336751,2.66e19,E,C0,t0,x0);  //LCM
