...
//-----------------------------------------------------------------------------
// Filename       : $RCSfile: N_DEV_InstanceName.C,v $
//
// Purpose        :
//
// Special Notes  :
...
InstanceName::decode()
{
  deviceType_ = decodeDeviceType();
  deviceName_ = decodeDeviceName();
  subcircuitName_ = decodeSubcircuitName();
  deviceLetter_ = decodeDeviceLetter();
  numInputs_ = decodeNumInputs();
}
...
std::string
InstanceName::decodeDeviceType() const
{
  // Skip subcircuit
...
std::string
InstanceName::decodeDeviceName() const
{
  // Skip subcuit
  std::string::size_type i = name_.find_last_of(':');
  i = (i == std::string::npos ? 0 : i + 1);
...
std::string
InstanceName::decodeSubcircuitName() const
{
  std::string::size_type i = name_.find_last_of(':');
...
char InstanceName::decodeDeviceLetter() const {
  // Skip subcircuit
  std::string::size_type i = name_.find_last_of(':');
  i = (i == std::string::npos ? 0 : i + 1);
...
int InstanceName::decodeNumInputs() const
{
  if (getDeviceLetter() == 'U') {
    std::string::size_type i = name_.find_last_of("!");
    if (i == std::string::npos)
      return 0;
...
std::string spiceInternalName(const InstanceName &entity_name, const std::string &lead)
{
  ...
  {
    s = s.substr(pos+1,1)+":"+s.substr(0,pos+1)+s.substr(pos+2);
  }
  return s;
}
