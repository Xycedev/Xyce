...
// ---------------------
//
// Revision Number: $Revision: 1.234.2.1 $
//
...
void setParameters(DeviceEntity &entity, std::vector<Param>::const_iterator begin, std::vector<Param>::const_iterator end, const DeviceOptions &device_options)
{
  ...
  for (std::vector<Param>::const_iterator param_it = begin; param_it != end; ++param_it)
  {
      ...
      if (param.given() || param.default_val())
      {
        ...
        else
        {
          if (descriptor.isType<double>())
          {
            if (!param.isNumeric())
            {
              UserError(entity) << "Cannot convert parameter " << tag <<  " to a numeric value from " << param.stringValue();
              continue;
            }
          ...
          else if (descriptor.isType<int>())
          {
            if (!param.isInteger())
            {
              UserError(entity) << "Cannot convert parameter " << tag << " to an integer value from " << param.stringValue();
              continue;
            }
          ...
          else if (descriptor.isType<bool>())
          {
            if (!param.isBool())
            {
              UserError(entity) << "Cannot convert parameter " << tag << " to a logical value from " << param.stringValue();
              continue;
            }
    ...
    else
    {
      ...
      if (!vc_stat)
      {
        DevelFatal(entity) << "Undefined parameter " << tag << ", this parameter is in metadata, but not recognized in constructor";
      }
    }
  }
