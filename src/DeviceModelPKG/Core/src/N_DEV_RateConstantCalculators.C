...
//
// Revision Date  : $Date: 2014/05/03 18:15:53 $
//
...
//-----------------------------------------------------------------------------
     ...
     double C0, double t0, double x0)
  {
    ...
    else
    {
      ...
      //reaction_distance_factor = 4*M_PI*1.40419676681412915e-4*(-ij);
      //reaction_distance_factor = 4*3.1416*1.404e-4*(-ij);
      Tdep=true;
    }
...
//-----------------------------------------------------------------------------
    ...
    :c0(C0)
  {
    ...
    {
      std::string msg;
      msg = "BourgoinCorbettCalculator: Invalid attempt to use rate method.";
      msg = "  This method is only valid for ternary reactions:\n";
      // exit with error --- we probably need to do something more careful
      // in parallel
      N_ERH_ErrorMgr::report(N_ERH_ErrorMgr::DEV_FATAL,msg);
    }
