...
// ---------------------
//
// Revision Number: $Revision: 1.127 $
//
// Revision Date  : $Date: 2014/07/31 19:56:16 $
//
...
void sourceFunctionMetadata(DeviceParamMap &sourceFcnMap)
{
    ...
    PHASEgiven(false)
{
  ...
  if (!(V0given && VAgiven && FREQgiven))
    Report::UserError() << "V0, VA and FREQ are required for the SIN source function";
    ...
    TAU2given (false)
{
  ...
  if (!(V1given && V2given))
    Report::UserError() << "V1 and V2 are required for the EXP source function";
    ...
    starttime_(0.0)
{
  ...
  if (NUM == 0)
    Report::UserError() << "At least one voltage/time pair must be specified for the PWL source function";
  if (REPEATTIME < 0 || REPEATTIME >=TVVEC[NUM-1].first)
    Report::UserError() << "PWL source's repeat value (R) must be >= 0 and < last value in Time-Voltage list";
...

{
  ...
  if (!(V0given && VAgiven))
    Report::UserError() << "V0 and VA are required for the SFFM source function";
