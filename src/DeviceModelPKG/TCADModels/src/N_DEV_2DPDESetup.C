...
void Instance::setupIntNameMap ()
{
  ...
  for (i=0;i<numMeshPoints;++i)
  {
    ...
    if (Vrow != -1)
    {
      ...
      oss << getName() << "_V_" << i << "_" << labelNameVector[i];
      intNameMap[Vrow] = oss.str();
    }

    if (Nrow != -1)
    {
      std::ostringstream oss;
      oss << getName() << "_N_" << i << "_" << labelNameVector[i];
      intNameMap[Nrow] = oss.str();
    }

    if (Prow != -1)
    {
      std::ostringstream oss;
      oss << getName() << "_P_" << i << "_" << labelNameVector[i];
      intNameMap[Prow] = oss.str();
    }
  }
}
//-----------------------------------------------------------------------------
