...
// ---------------------
//
// Revision Number: $Revision: 1.130 $
//
// Revision Date  : $Date: 2014/08/07 23:08:53 $
//
...
void Traits::loadInstanceParameters(ParametricData<DiodePDE::Instance> &p)
{
  ...
  p.addPar("OUTPUTINTERVAL", 0.0, &DiodePDE::Instance::outputInterval)
    ...
    .setDescription("Time interval for tecplot output (if enabled).");
  ...
  p.addPar("JUNCTIONAREA", 1.0e-4, &DiodePDE::Instance::junctionArea)
    .setDescription("Cross sectional area used to compute recombination integral.");
  ...
  p.addPar("SRH", true, &DiodePDE::Instance::includeSRHRecomb)
    .setUnit(U_LOGIC)
    .setDescription("Flag to turn on/off Shockley-Reed-Hall recombination.");

  p.addPar ("FERMIDIRAC", false, false, NO_DEP, &DiodePDE::Instance::fermiDiracFlag,
      NULL, U_NONE, CAT_UNKNOWN, "Use Fermi-Dirac statistics.");
...
// Doping file params:
  ...
  p.addPar("NX", 11, &DiodePDE::Instance::NX)
    ...
    .setDescription("Number of mesh points (if a single material).");
  ...
  if (intNameMap.empty ())
  {
    ...
    for (itmp=0;itmp<NX;++itmp)
    {
      if (li_Vrowarray[itmp] != -1)
      {
        ...
        oss << getName() << "_V_" << itmp;
        intNameMap[li_Vrowarray[itmp]] = oss.str();
      }

      if (li_Nrowarray[itmp] != -1)
      {
        std::ostringstream oss;
        oss << getName() << "_N_" << itmp;
        intNameMap[li_Nrowarray[itmp]] = oss.str();
      }

      if (li_Prowarray[itmp] != -1)
      {
        std::ostringstream oss;
        oss << getName() << "_P_" << itmp;
        intNameMap[li_Prowarray[itmp]] = oss.str();
      }
    }
  }
...
#if 0
    Xyce::dout() << "ihet="<<ihet<< "  perm1/dx1="<<perm1/dx1<<"  perm2/dx2="<<perm2/dx2;
    Xyce::dout() << "  Vrow="<<Vrow<<"  Nrow="<<Nrow<<"  Prow="<<Prow<<std::endl;
#endif
...
bool Instance::calcDopingProfile ()
{
  ...
  if (Na == 0.0 || Nd == 0.0)
  {
    UserError(*this) << "Mistake in doping. Na=" << Na << " %12.4e  Nd=" << Nd;
    bsuccess = false;
  }
...
bool Instance::setupMesh ()
{
  if(layerCompositeSpecified)
  {
    ...
    for (int imat=0;imat<matVecSize;++imat)
    {
      ...
      Xyce::dout() << "name="<<matLay.name <<"  NX="<<matLay.NX<<" LX="<<matLay.LX <<std::endl;
